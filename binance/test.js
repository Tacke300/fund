...
import { exec } from 'child_process'; // ĐÃ SỬA LỖI CÚ PHÁP IMPORT
import fs from 'fs';
...
async function closePosition(symbol, quantity, reason, positionSide) {
    ...
    try {
        ...
        if (posOnBinance) {
            // Hủy lệnh TP/SL chờ của vị thế này trước khi đóng
...
async function openMarketPosition(symbol, tradeDirection, usdtBalance, maxLeverage) {
    ...
    try {
        ...
        return {
            ...
            // THUỘC TÍNH MỚI CHO LOGIC ĐÓNG MỘT PHẦN / ĐIỀU CHỈNH SL
            partialClosePrices: [], // Lưu giá entry của lệnh lỗ khi đóng một phần
            closedQuantity: 0, // Tổng số lượng lệnh lỗ đã đóng một phần
            hasAdjustedSL6thClose: false, // Cờ hiệu để kiểm tra điều chỉnh SL lần 6
            hasAdjustedSL8thClose: false, // Cờ hiệu để kiểm tra điều chỉnh SL lần 8
        };
    } catch (error) {
...
async function setInitialTPAndSL(position) {
    ...
    try {
        ...
        // Các thuộc tính liên quan đến partialClose/SL adjustment đã được khởi tạo trong openMarketPosition
        position.closedAmount = 0; // Legacy, có thể xóa
        position.partialCloseLevels = []; // Legacy, có thể xóa
        position.nextPartialCloseIndex = 0; // Legacy, có thể xóa
        position.closedLossAmount = 0; // Legacy, có thể xóa
        position.nextPartialCloseLossIndex = 0; // Legacy, có thể xóa
        position.hasAdjustedSLTo200PercentProfit = false; // Legacy, có thể xóa
        position.hasAdjustedSLTo500PercentProfit = false; // Legacy, có thể xóa
...
// HÀM MỚI: Đóng một phần vị thế
async function closePartialPosition(position, quantityToClose) {
    if (!position || quantityToClose <= 0) return;
    const symbolDetails = await getSymbolDetails(position.symbol);
    if (!symbolDetails) {
        addLog(`Lỗi: Không tìm thấy chi tiết symbol cho ${position.symbol}. Không thể đóng một phần lệnh.`);
        return;
    }
    ...
    if (quantityToClose === 0 || quantityToClose > Math.abs(position.quantity)) {
        addLog(`Cảnh báo: Số lượng đóng một phần lệnh ${position.side} không hợp lệ: ${quantityToClose}.`);
        return;
    }

    addLog(`Đang đóng một phần lệnh ${position.side} ${position.symbol}: ${quantityToClose.toFixed(symbolDetails.quantityPrecision)}`);
    ...
    try {
        ...
        position.closedQuantity += quantityToClose;
        position.partialClosePrices.push(position.entryPrice); // Lưu giá entry của lệnh lỗ tại thời điểm đóng một phần
        
        addLog(`Đã đóng một phần lệnh ${position.side}. Số lượng còn lại: ${position.quantity.toFixed(symbolDetails.quantityPrecision)}`);

    } catch (error) {
        addLog(`Lỗi khi đóng một phần lệnh ${position.side}: ${error.msg || error.message}`);
    }
}

// HÀM MỚI: Mở thêm vị thế (cân bằng lại lệnh lỗ)
async function addPosition(position, quantityToAdd, reason) {
    if (!position || quantityToAdd <= 0) return;
    const symbolDetails = await getSymbolDetails(position.symbol);
    if (!symbolDetails) {
        addLog(`Lỗi: Không tìm thấy chi tiết symbol cho ${position.symbol}. Không thể mở thêm lệnh.`);
        return;
    }

    // Làm tròn số lượng theo stepSize của sàn
    quantityToAdd = parseFloat((Math.floor(quantityToAdd / symbolDetails.stepSize) * symbolDetails.stepSize).toFixed(symbolDetails.quantityPrecision));

    if (quantityToAdd === 0) {
        addLog(`Cảnh báo: Số lượng mở thêm lệnh ${position.side} không hợp lệ: ${quantityToAdd}.`);
        return;
    }

    addLog(`Đang mở thêm lệnh ${position.side} ${position.symbol}: ${quantityToAdd.toFixed(symbolDetails.quantityPrecision)} (Lý do: ${reason})`);

    try {
        const orderSide = (position.side === 'LONG') ? 'BUY' : 'SELL'; // Ngược lại với side của vị thế để mở thêm
        await callSignedAPI('/fapi/v1/order', 'POST', {
            symbol: position.symbol,
            side: orderSide,
            positionSide: position.side,
        ...
        // Thực tế, khi mở thêm, giá entry trung bình của vị thế sẽ thay đổi.
        // Cần cập nhật position.entryPrice nếu bạn muốn theo dõi giá trung bình.
        // Ví dụ đơn giản: position.entryPrice = (position.entryPrice * (position.quantity - quantityToAdd) + currentPrice * quantityToAdd) / position.quantity;
        // Nhưng việc này phức tạp hơn với margin và PNL, nên tạm thời không cập nhật entryPrice nếu bot chỉ dùng entryPrice ban đầu làm mốc.
        addLog(`Đã mở thêm lệnh ${position.side}. Số lượng mới: ${position.quantity.toFixed(symbolDetails.quantityPrecision)}`);

    } catch (error) {
        addLog(`Lỗi khi mở thêm lệnh ${position.side}: ${error.msg || error.message}`);
    }
}

// HÀM MỚI: Cập nhật lệnh Stop Loss (hủy cũ, đặt mới)
async function updatePositionSL(position, newSLPrice) {
    if (!position || !position.symbol) return;
    addLog(`Đang cập nhật SL cho vị thế ${position.side} về giá ${newSLPrice.toFixed(position.pricePrecision)}`);
    try {
        // Hủy tất cả lệnh chờ cũ của vị thế này (bao gồm SL cũ và TP cũ)
        await cancelOpenOrdersForSymbol(position.symbol, position.side);
        
        // Lấy thông tin vị thế hiện tại trên Binance để lấy quantity chính xác
        const positionsOnBinance = await callSignedAPI('/fapi/v2/positionRisk', 'GET', { symbol: position.symbol });
        ...
        const orderSide = (position.side === 'LONG') ? 'SELL' : 'BUY';
        // Đặt lệnh SL mới với quantity hiện tại của vị thế
        const newSlOrder = await callSignedAPI('/fapi/v1/order', 'POST', {
            ...
            quantity: Math.abs(parseFloat(currentPosOnBinance.positionAmt)),
        });
        position.currentSLId = newSlOrder.orderId;
        position.initialSLPrice = newSLPrice; // Cập nhật giá SL trong trạng thái bot
        addLog(`Đã đặt SL mới cho ${position.side}: ${newSLPrice.toFixed(position.pricePrecision)}`);

    } catch (error) {
        addLog(`Lỗi khi cập nhật SL cho ${position.side}: ${error.msg || error.message}`);
    }
}
...
const manageOpenPosition = async () => {
    ...
    try {
        ...
        let longPosOnBinance = positions.find(p => p.positionSide === 'LONG' && parseFloat(p.positionAmt) > 0);
        let shortPosOnBinance = positions.find(p => p.positionSide === 'SHORT' && parseFloat(p.positionAmt) < 0);

        // Cập nhật trạng thái vị thế cục bộ dựa trên Binance
        if (currentLongPosition && !longPosOnBinance) currentLongPosition = null; 
        if (currentShortPosition && !shortPosOnBinance) currentShortPosition = null; 

        if (longPosOnBinance && currentLongPosition) { 
            ...
            currentLongPosition.quantity = Math.abs(parseFloat(longPosOnBinance.positionAmt)); // Cập nhật quantity thực tế
        }
        if (shortPosOnBinance && currentShortPosition) { 
            ...
            currentShortPosition.quantity = Math.abs(parseFloat(shortPosOnBinance.positionAmt)); // Cập nhật quantity thực tế
        }

        if (!longPosOnBinance && !shortPosOnBinance && botRunning) {
        ...
        if (winningPos && losingPos) {
            ...
            // LOGIC 1: Đóng một phần lệnh lỗ khi lệnh lãi đạt ngưỡng
            if (nextLossCloseLevel && currentProfitPercentage >= nextLossCloseLevel) {
                ...
                const quantityToClose = losingPos.initialQuantity * 0.1; 
                await closePartialPosition(losingPos, quantityToClose);
                winningPos.nextPartialCloseLossIndex++; // Tăng chỉ số để kiểm tra ngưỡng tiếp theo
            }

            // LOGIC 2: Điều chỉnh SL khi đạt ngưỡng đóng một phần lệnh lỗ
            // Lưu ý: winningPos.nextPartialCloseLossIndex ở đây đã tăng sau khi đóng một phần
            const partialCloseCount = winningPos.nextPartialCloseLossIndex; // Số lần đã đóng một phần

            if (partialCloseCount >= 6 && !winningPos.hasAdjustedSL6thClose) {
                // Điều chỉnh SL cả 2 lệnh về giá đóng 1 phần lệnh lỗ lần thứ 2
                const slTargetPrice = losingPos.partialClosePrices[1]; // Index 1 là lần đóng thứ 2
                if (slTargetPrice !== undefined) {
                    addLog(`Đạt mốc đóng lỗ lần ${partialCloseCount}. Điều chỉnh SL của cả 2 lệnh về giá đóng lỗ lần 2 (${slTargetPrice.toFixed(losingPos.pricePrecision)}).`);
                    await updatePositionSL(currentLongPosition, slTargetPrice);
                    await updatePositionSL(currentShortPosition, slTargetPrice);
                    winningPos.hasAdjustedSL6thClose = true; // Đặt cờ hiệu đã điều chỉnh
                } else {
                    addLog(`Cảnh báo: Không tìm thấy giá đóng lỗ lần 2 để điều chỉnh SL.`);
                }
            }
            
            if (partialCloseCount >= 8 && !winningPos.hasAdjustedSL8thClose) {
                // Đóng lệnh lỗ và điều chỉnh SL lệnh lãi về giá đóng 1 phần lệnh lỗ lần thứ 5
                addLog(`Đạt mốc đóng lỗ lần ${partialCloseCount}. Đóng hoàn toàn lệnh ${losingPos.side} (lệnh lỗ).`);
                await closePosition(losingPos.symbol, losingPos.quantity, 'Đóng hoàn toàn lệnh lỗ theo quy tắc', losingPos.side);
                
                const slTargetPrice = losingPos.partialClosePrices[4]; // Index 4 là lần đóng thứ 5
                if (slTargetPrice !== undefined) {
                    addLog(`Điều chỉnh SL của lệnh lãi ${winningPos.side} về giá đóng lỗ lần 5 (${slTargetPrice.toFixed(winningPos.pricePrecision)}).`);
                    await updatePositionSL(winningPos, slTargetPrice);
                    winningPos.hasAdjustedSL8thClose = true; // Đặt cờ hiệu đã điều chỉnh
                } else {
                    addLog(`Cảnh báo: Không tìm thấy giá đóng lỗ lần 5 để điều chỉnh SL lệnh lãi.`);
                }
            }

            // LOGIC 3: Mở thêm lệnh lỗ nếu giá quay về giá ban đầu mở lệnh
            // Chỉ thực hiện từ lần đóng 1 phần thứ 1 đến 7
            if (losingPos.closedQuantity > 0 && partialCloseCount <= 7) { 
                const currentPrice = losingPos.currentPrice; // Lấy giá thị trường hiện tại của lệnh lỗ
                const priceThreshold = losingPos.entryPrice * 0.0005; // Ngưỡng nhỏ, ví dụ 0.05% của giá vào
                
                if (Math.abs(currentPrice - losingPos.entryPrice) <= priceThreshold) {
                    addLog(`Giá lệnh lỗ ${losingPos.side} đã về gần giá mở ban đầu (${losingPos.entryPrice.toFixed(losingPos.pricePrecision)} vs ${currentPrice.toFixed(losingPos.pricePrecision)}). Đang mở lại ${losingPos.closedQuantity.toFixed(losingPos.quantityPrecision)} khối lượng.`);
                    await addPosition(losingPos, losingPos.closedQuantity, 'Cân bằng lại lệnh lỗ');
                    losingPos.closedQuantity = 0; // Reset số lượng đã đóng một phần sau khi mở lại
                    losingPos.partialClosePrices = []; // Reset danh sách giá đóng một phần
                    winningPos.nextPartialCloseLossIndex = 0; // Reset số lần đóng một phần
                    winningPos.hasAdjustedSL6thClose = false; // Reset cờ hiệu điều chỉnh SL
                    winningPos.hasAdjustedSL8thClose = false; // Reset cờ hiệu điều chỉnh SL

                    // Cần đặt lại TP/SL cho cả hai vị thế sau khi cân bằng lại
                    addLog(`Đã cân bằng lại lệnh lỗ. Đang đặt lại TP/SL cho cả hai vị thế.`);
                    await setInitialTPAndSL(currentLongPosition);
                    await setInitialTPAndSL(currentShortPosition);
                }
            }
        }
    } catch (error) {
