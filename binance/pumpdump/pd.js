...
// ============ CHIẾN THUẬT DIỆT LỆNH 2 TẦNG (THEO Ý M) ============
async function forceClearAllOrders(symbol) {
    if (clearingSymbols.has(symbol)) return;
    clearingSymbols.add(symbol);
    try {
        addBotLog(`💣 [${symbol}] Kích hoạt forceClear 2 tầng...`);

        // TẦNG 1: Bulk Delete - Binance tự quét và xóa sạch
        try {
            await binancePrivate('/fapi/v1/allOpenOrders', 'DELETE', { symbol });
            addBotLog(`💥 Tầng 1: Đã gửi DELETE allOpenOrders`);
        } catch (e) { addBotLog(`⚠️ Tầng 1 (Bulk) lỗi: ${e.message}`, "warning"); }

        // Chờ sàn cập nhật trạng thái
        await new Promise(r => setTimeout(r, 1200));

        // TẦNG 2: Quét ID - Nếu còn sót bất cứ lệnh nào (kể cả lệnh ẩn) thì diệt đích danh
        const openOrders = await binancePrivate('/fapi/v1/openOrders', 'GET', { symbol });
        if (openOrders.length > 0) {
            addBotLog(`🔍 Tầng 2: Phát hiện ${openOrders.length} lệnh cứng đầu. Diệt theo ID...`);
            for (const order of openOrders) {
                try {
                    await binancePrivate('/fapi/v1/order', 'DELETE', { symbol, orderId: order.orderId });
                    addBotLog(`🗑️ Đã diệt ID: ${order.orderId}`, "success");
                } catch (err) {}
            }
        } else { addBotLog(`✅ Tầng 2: Sàn đã sạch bóng lệnh.`); }

        await new Promise(r => setTimeout(r, 1000));
    } catch (e) { addBotLog(`🚨 Lỗi forceClear: ${e.message}`, "error"); }
...
async function syncTPSL(symbol, side, entry, info, qty, customTP = null, customSL = null) {
    ...
    try {
        ...
        addBotLog(`🎯 [${symbol}] TPSL OK | TP ID: ${orderTP.id} | SL ID: ${orderSL.id}`, "success");
        return { tp: parseFloat(tpPrice), sl: parseFloat(slPrice) };
    } catch (e) {
        addBotLog(`❌ [${symbol}] Lỗi TPSL: ${e.message}. Đang dọn rác...`, "error");
        await forceClearAllOrders(symbol);
        return { tp: 0, sl: 0 };
    }
}

// ============ MỞ LỆNH VÀ DCA (TÍCH HỢP FORCE CLEAR) ============
async function openPosition(symbol, isDCA = false, manualData = null) {
    ...
    try {
        // LUÔN DỌN SẠCH TRƯỚC KHI MỞ LỆNH HOẶC DCA
        addBotLog(`🛡️ [${symbol}] Đang dọn dẹp trước khi ${isDCA ? 'DCA' : 'mở lệnh'}...`);
        await forceClearAllOrders(symbol);
    ...
    try {
        ...
        for (let [key, botPos] of botActivePositions) {
            if (!exchangeKeys.has(key)) {
                addBotLog(`📉 [${botPos.symbol}] Đã đóng. Đang forceClear rác...`);
...
async function init() {
    try {
        ...
        status.isReady = true;
        addBotLog("👹 LUFFY V10 - 2-STAGE GHOST KILLER", "success");
        priceMonitorLoop();
...
APP.post('/api/test', async (req, res) => {
    ...
    try {
        ...
        if (action === 'clear') {
            if (parseInt(mode) === 5) await forceClearAllOrders(symbol);
            else {
                // Giữ mode cũ nếu cần
                await binancePrivate('/fapi/v1/allOpenOrders', 'DELETE', { symbol });
            }
