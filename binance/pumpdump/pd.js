        ...
        for (let [key, b] of botActivePositions) {
            ...
            if (realP) {
                ...
                if (netPnl > 0 || b.side === 'LONG') {
                    ...
                    addBotLog(`💰 BOT CHỐT ${b.symbol} | Net: ${netPnl.toFixed(2)}$`, "success");
                } else {
...
// --- API KHỚP VỚI FETCH TRONG HTML CỦA ÔNG ---
APP.get('/api/status', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            status: {
                ...status,
                // Giảm trừ thời gian blacklist để HTML hiển thị đếm ngược (giây)
                blackList: Object.fromEntries(Object.entries(status.blackList).map(([s, t]) => [s, Math.max(0, Math.floor((t - Date.now()) / 1000))]))
            }, 
            wallet: { 
                totalWalletBalance: acc ? parseFloat(acc.totalWalletBalance).toFixed(2) : "0.00", 
                availableBalance: acc ? parseFloat(acc.availableBalance).toFixed(2) : "0.00", 
...
APP.post('/api/settings', (req, res) => { 
    ...
    // Ép kiểu để không bị lỗi so sánh chuỗi
    if (botSettings.maxDCA) botSettings.maxDCA = parseInt(botSettings.maxDCA);
    if (botSettings.maxPositions) botSettings.maxPositions = parseInt(botSettings.maxPositions);
    if (botSettings.minVol) botSettings.minVol = parseFloat(botSettings.minVol);
    
    addBotLog(`⚙️ Đã nhận cấu hình mới`, "success");
        ...
        if (order) {
            ...
            if (p) {
                ...
                botActivePositions.set(`${symbol}_${side}`, { 
                    ...
                    currentMargin: margin, currentQty: Math.abs(parseFloat(p.positionAmt)), 
                    pnl: 0, priceDev: 0, hitTime: null 
                });
                addBotLog(`✅ BOT Mở ${symbol} ${side}`);
            }
        }
...
async function init() {
    try {
        const ipRes = await axios.get('https://api.ipify.org?format=json').catch(() => ({ data: { ip: "IPv4 Error" } }));
        console.log(`\n🌍 Server IP: ${ipRes.data.ip}`);
        ...
        addBotLog(`🚀 Hệ thống sẵn sàng (IP: ${ipRes.data.ip})`);
    } catch (e) { 
        console.log("Init Error, retrying in 5s...");
...
// Lấy dữ liệu từ Bot Scanner (Cổng 9000)
setInterval(() => {
...
APP.listen(9001, () => console.log("Dashboard bot chạy tại http://localhost:9001"));
