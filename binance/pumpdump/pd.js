...
// --- LOGIC THỜI GIAN CHUẨN 7H SÁNG (UTC+7) ---
    ...
    try {
        ...
        for (const symbol of botManagedSymbols) {
            ...
            if (!hasTP || !hasSL) {
                ...
                const rate = (lev < 26 ? 1.11 : 2.22) / lev; // Logic TP/SL của bạn
...
// --- 2. DỌN DẸP VỊ THẾ ĐÃ ĐÓNG (GIẢI PHÓNG SLOT) ---
...
async function hunt() {
    ...
    for (const c of candidates) {
        ...
        try {
            ...
            setTimeout(enforceTPSL, 2000); // Cài TP/SL sau 2s
        } catch (e) { console.log(`❌ Lỗi vào lệnh ${c.symbol}`); }
    }
    isProcessing = false;
}

// --- 4. WEBSOCKET DỮ LIỆU SÀN ---
function initWS() {
    ...
    ws.on('message', (data) => {
        ...
        tickers.forEach(t => {
            ...
            if (h && h.status === 'PENDING') {
                ...
                if ((h.type === 'UP' && diff >= 5) || (h.type === 'DOWN' && diff <= -5)) h.status = 'WIN';
                else if ((h.type === 'UP' && diff <= -5) || (h.type === 'DOWN' && diff >= 5)) h.status = 'LOSE';
            }
        });
    });
...
// --- 5. EXPRESS API & HTML (KHÔI PHỤC PIRATE ENGINE) ---
...
app.get('/api/status', async (req, res) => {
    try {
        ...
        const active = pos.filter(p => parseFloat(p.positionAmt) !== 0).map(p => {
            return { symbol: p.symbol, side: p.positionSide, entry: p.entryPrice, mark: p.markPrice, pnl: parseFloat(p.unrealizedProfit).toFixed(2) };
        });
        res.json({
...
app.get('/', (req, res) => {
    ...
    <div class="max-w-6xl mx-auto space-y-4">
        <div class="flex justify-between items-center bg-[#111] p-4 border-b-2 border-red-600 rounded">
            <h1 class="luffy text-4xl italic text-white uppercase">Luffy Pirate Bot v5</h1>
            <div class="flex gap-10">
                <div class="text-center"><p class="text-[10px] text-gray-500">WIN (7AM)</p><p id="win" class="text-2xl font-bold up">0</p></div>
                <div class="text-center"><p class="text-[10px] text-gray-500">LOSE (7AM)</p><p id="lose" class="text-2xl font-bold down">0</p></div>
            </div>
        </div>
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 md:col-span-3 bg-[#111] p-4 rounded space-y-3">
                <h2 class="text-xs font-bold text-red-500 uppercase italic">Cài đặt buồm</h2>
                <div><label class="text-[10px]">MAX SLOT</label><input type="number" id="maxPositions" value="3"></div>
                ...
                <div><label class="text-[10px]">KIỂU VỐN</label><select id="invType"><option value="percent">% Ví</option><option value="fixed">$ Fixed</option></select></div>
                <button id="btn" onclick="save()" class="w-full p-4 font-bold uppercase rounded mt-2 transition-all"></button>
            </div>
            <div class="col-span-12 md:col-span-9 bg-[#111] rounded overflow-hidden">
                <div class="p-2 bg-zinc-900 text-[10px] font-bold italic">HẢI QUÂN ĐANG TRUY ĐUỔI (POSITIONS)</div>
                <table class="w-full text-left text-xs"><thead class="bg-black/50"><tr><th class="p-3">MÃ</th><th class="p-3">BÊN</th><th class="p-3">ENTRY</th><th class="p-3 text-right">PNL ($)</th></tr></thead><tbody id="posBody"></tbody></table>
            </div>
            <div class="col-span-12 bg-[#111] rounded overflow-hidden">
                <div class="p-2 bg-zinc-900 text-[10px] font-bold italic">NHẬT KÝ RA KHƠI (HISTORY)</div>
                <table class="w-full text-left text-[11px]"><thead class="bg-black/50"><tr><th class="p-2">GIỜ</th><th class="p-2">MÃ</th><th class="p-2">VOL</th><th class="p-2">TYPE</th><th class="p-2 text-right">KẾT QUẢ</th></tr></thead><tbody id="histBody"></tbody></table>
            </div>
        </div>
    </div>
    <script>
        async function update(){
            ...
            document.getElementById('posBody').innerHTML = d.activePositions.map(p=>\`<tr class="border-b border-zinc-900"><td class="p-3 font-bold">\${p.symbol}</td><td class="p-3 \${p.side==='LONG'?'up':'down'} font-black">\${p.side}</td><td class="p-3">\${p.entry}</td><td class="p-3 text-right font-bold \${p.pnl>=0?'up':'down'}">\${p.pnl}</td></tr>\`).join('');
            document.getElementById('histBody').innerHTML = d.history.map(h=>\`<tr class="border-b border-zinc-900/50"><td class="p-2 text-gray-500">\${new Date(h.startTime).toLocaleTimeString()}</td><td class="p-2 font-bold \${h.type==='UP'?'up':'down'}">\${h.symbol}</td><td class="p-2">\${h.max1}%</td><td class="p-2">\${h.type}</td><td class="p-2 text-right font-bold \${h.status==='WIN'?'up':(h.status==='LOSE'?'down':'')}">\${h.status}</td></tr>\`).join('');
        }
        async function save(){
            const r = await fetch('/api/status'); const d = await r.json();
            const body = { isRunning: !d.botSettings.isRunning, maxPositions: parseInt(document.getElementById('maxPositions').value), minVol: parseFloat(document.getElementById('minVol').value), invValue: parseFloat(document.getElementById('invValue').value), invType: document.getElementById('invType').value };
        ...
        r.on('end', () => {
            try {
                ...
                info.symbols.forEach(s => {
                    ...
                    exchangeInfo[s.symbol] = { quantityPrecision: s.quantityPrecision, pricePrecision: s.pricePrecision, stepSize: parseFloat(lot.stepSize) };
                });
                isInitializing = false;
                console.log("✅ Dữ liệu sàn OK. Bot Pirate sẵn sàng!");
...
init(); initWS();
setInterval(hunt, 2000); // Săn lệnh 2s/lần
setInterval(enforceTPSL, 5000); // Check TP/SL 5s/lần
setInterval(cleanupClosedPositions, 10000); // Dọn slot 10s/lần
setInterval(() => { fs.writeFileSync(HISTORY_FILE, JSON.stringify(Object.fromEntries(historyMap))); }, 60000);

app.listen(PORT, '0.0.0.0', () => console.log(`🚢 Luffy Pirate Port ${PORT}`));
