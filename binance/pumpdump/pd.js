    ...
    try {
        ...
        status.activePositions = positions
            ...
            .map(p => ({
                ...
                pnlPercent: parseFloat(p.unRealizedProfit) >= 0 ? 
                    ((parseFloat(p.unRealizedProfit) / parseFloat(p.isolatedWallet)) * 100).toFixed(2) : 
                    ((parseFloat(p.unRealizedProfit) / parseFloat(p.isolatedWallet)) * 100).toFixed(2)
            }));
    ...
    for (const c of status.candidatesList) {
        ...
        if (maxVol >= botSettings.minVol) {
            ...
            try {
                ...
                // Set Leverage mặc định 20x
                await callBinance('/fapi/v1/leverage', 'POST', { symbol: c.symbol, leverage: 20 });
                
                const info = status.exchangeInfo[c.symbol];
                const qty = ((status.currentBalance * 0.1) / c.c1).toFixed(info.quantityPrecision); // Giả định margin đơn giản
                ...
                addBotLog(`🚢 GIƯƠNG BUỒM: ${c.symbol} [${posSide}]`, "success");
            } catch (e) { addBotLog(`Lỗi lệnh ${c.symbol}`, "error"); }
            isProcessing = false;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .avatar-img { width: 100%; height: 100%; border-radius: 12px; border: 2px solid var(--luffy-red); background: #1a1a1a; }
...
<body class="p-2 md:p-6">
        ...
        <div class="md:col-span-4 flex flex-col gap-4 overflow-hidden">
            <div class="card flex-grow flex flex-col overflow-hidden border-t-4 border-blue-500">
                <div class="p-3 border-b border-white/5 bg-blue-500/5 flex justify-between items-center">
                    ...
                    <button onclick="clearLogs()" class="text-[9px] text-gray-600">Xóa</button>
                </div>
    ...
    <script>
        ...
        function clearLogs() { document.getElementById('botLogs').innerHTML = ''; }
...
APP.get('/api/status', (req, res) => res.json({ botSettings, status, activePositions: status.activePositions }));
...
async function startBot() {
    ...
    // Sync candidates from Server 9000
    setInterval(() => {
