...
// Khởi tạo CCXT với cấu hình ép buộc Futures
const exchange = new ccxt.binance({ 
    apiKey: API_KEY, 
    secret: SECRET_KEY, 
    enableRateLimit: true, 
    options: { 
        defaultType: 'future', // Ép dùng Futures
...
async function priceMonitor() {
    ...
    try {
        // Dùng hàm chuyên biệt fapiPrivate để tránh nhầm sang Spot
    ...
    try {
        ...
        if (order) {
            ...
            if (p) {
                ...
                botActivePositions.set(`${symbol}_${side}`, { symbol, side, entryPrice: entry, tp: sync.tp, sl: sync.sl, dcaCount: dcaData ? dcaData.dcaCount : 0, leverage: info.maxLeverage, firstEntry: firstE, firstMargin: dcaData ? dcaData.firstMargin : margin, currentQty: Math.abs(parseFloat(p.positionAmt)), pnl: 0, priceDev: 0, hitTime: null });
...
async function syncTPSL(symbol, side, info, tpPrice, slPrice) {
    ...
    try {
        ...
        for (const o of orders.filter(o => o.positionSide === side)) { await exchange.fapiPrivateDeleteOrder({ symbol, orderId: o.orderId }); }
...
async function init() {
    try {
        addBotLog(`🔄 Đang khởi tạo...`);
        // Ép CCXT sử dụng Futures cho cả hàm loadMarkets
        ...
        info.symbols.forEach(s => {
            ...
            temp[s.symbol] = { quantityPrecision: s.quantityPrecision, pricePrecision: s.pricePrecision, stepSize: parseFloat(lotFilter.stepSize), maxLeverage: b?.brackets[0]?.initialLeverage || 20 };
        });
