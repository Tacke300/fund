...
async function callBinance(endpoint, method = 'GET', params = {}, retries = 3) {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (res.code && res.code !== 200) addBotLog(`❌ Lỗi API (${endpoint}): ${res.msg}`, "error");
            return res;
        } catch (e) { if (i === retries - 1) throw e; await sleep(400); }
    }
}

// 🚀 MỞ VỊ THẾ - LOGIC ĐẶT LỆNH MỚI
async function openPosition(symbol, side, info, signals) {
    ...
    try {
        ...
        if (margin > available) {
            addBotLog(`❌ Thất bại ${symbol}: Thiếu vốn (Cần ${margin.toFixed(2)}$, có ${available.toFixed(2)}$)`, "error");
            return;
        }
        ...
        if (order.orderId) {
            addBotLog(`⚡ Khớp Market ${symbol}. Đợi 1s để lấy giá khớp thật...`, "info");
            await sleep(1000); 

            // 3. Lấy giá khớp thực tế để đặt TP/SL chuẩn
