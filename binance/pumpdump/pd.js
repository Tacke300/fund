...
// Khởi tạo settings khớp chính xác với ID trong HTML của ông
let botSettings = { 
    isRunning: false, 
    maxPositions: 10, 
    invValue: 1.5, 
...
// Cấu trúc status chuẩn để HTML sync() không bị lỗi undefined
let status = { 
    currentBalance: 0, 
    botLogs: [], 
    exchangeInfo: {}, 
...
let history = []; // Để tính Winrate trên HTML
    ...
    return new Promise((resolve, reject) => {
        const req = https.request(url, { method, headers: { 'X-MBX-APIKEY': API_KEY }, timeout: 8000 }, res => {
            ...
            res.on('end', () => {
                ...
                } catch (e) { reject({ msg: "Sàn phản hồi lỗi JSON" }); }
            });
        });
...
// HÀM GHIM TP/SL - FIX LỖI "Algo Order"
    ...
    try {
        ...
        for (const p of active) {
            ...
            if (!hasTP || !hasSL) {
                ...
                // LỆNH ALGO: KHÔNG ĐƯỢC GỬI QUANTITY KHI CÓ CLOSEPOSITION=TRUE
                if (!hasTP) {
                ...
                addBotLog(`🛡️ Ghim TP/SL chuẩn: ${symbol}`, "success");
            }
        }
    } catch (e) { console.log("Lỗi tuần tra:", e.msg || ""); }
}

async function hunt() {
    if (isInitializing) return;
    try {
        // CẬP NHẬT SỐ DƯ LIÊN TỤC CHO HTML
        ...
        for (const c of status.candidatesList) {
            ...
            try {
                ...
                addBotLog(`🚀 VÀO LỆNH: ${side} ${c.symbol}`, "success");
                setTimeout(enforceTPSL, 5000); // Đợi 5s đúng ý ông
            } catch (err) {}
        }
    } catch (e) {}
}

// QUÉT KÈO LIÊN TỤC 3S/LẦN
...
// ĐIỂM KẾT NỐI VỚI HTML: TRẢ VỀ ĐÚNG CẤU TRÚC
APP.get('/api/status', async (req, res) => {
    try {
        ...
        // Trả về object chứa mọi thứ HTML cần
    ...
    https.get('https://fapi.binance.com/fapi/v1/exchangeInfo', (r) => {
        ...
        r.on('end', () => {
            try {
                ...
                addBotLog("⚓ Sẵn sàng xuất kích!", "success");
            } catch (e) {}
        });
...
init();
setInterval(fetchCandidates, 3000); // Quét radar 3s
setInterval(hunt, 5000); // Tìm kèo 5s
setInterval(enforceTPSL, 15000); // Kiểm tra TP/SL 15s
