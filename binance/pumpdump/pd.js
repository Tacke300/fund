...
let botSettings = { 
    ...
    minVol: 0.5, // Đây là giá trị biến động tối thiểu để vào lệnh
    posTP: 1.0, posSL: 3.0 
};
...
async function openPosition(symbol, side, price, info, scan) {
    try {
        ...
        if (parseFloat(qty) * price < 5.0) return addBotLog(`⚠️ Vốn quá thấp cho ${symbol}`, "warn");
        ...
        if (order.orderId) {
            ...
            await callBinance('/fapi/v1/order', 'POST', { symbol, side: side === 'BUY' ? 'SELL' : 'BUY', positionSide: posSide, type: 'TAKE_PROFIT_MARKET', stopPrice: tp, closePosition: 'true' });
            await callBinance('/fapi/v1/order', 'POST', { symbol, side: side === 'BUY' ? 'SELL' : 'BUY', positionSide: posSide, type: 'STOP_MARKET', stopPrice: sl, closePosition: 'true' });
            
            activeOrdersTracker.set(symbol, { entryPrice: price, margin: margin.toFixed(2), side: posSide, tpPrice: tp, slPrice: sl, snapshot: scan });
            addBotLog(`🚀 MỞ ${symbol} [${posSide}] thành công!`, "success");
        }
...
async function mainLoop() {
    ...
    try {
        ...
        // Xóa tracker nếu lệnh đã đóng trên sàn
        for (const [sym, data] of activeOrdersTracker) {
            if (!activePositions.some(p => p.symbol === sym)) activeOrdersTracker.delete(sym);
        }
        ...
        for (const coin of status.candidatesList) {
            ...
            // LOGIC GỐC: Dùng maxV (Biến động mạnh nhất trong 3 khung)
    ...
    } catch (e) { console.log("Lỗi quét: ", e); }
}

// Lấy dữ liệu từ Scanner Port 9000
setInterval(() => {
