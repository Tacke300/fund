...
async function syncTPSL(symbol, side, entry, info, customTP = null, customSL = null) {
    ...
    try {
        ...
        addBotLog(`🎯 [${symbol}] TPSL Mới: TP ${tpPrice} | SL ${slPrice}`, "success");
...
async function openPosition(symbol, isDCA = false, manualData = null) {
    ...
    try {
        ...
        if (order) {
            ...
            if (realP) {
                const sync = await syncTPSL(symbol, 'SHORT', parseFloat(realP.entryPrice), info, manualData?.tp, manualData?.sl);
                botActivePositions.set(posKey, { 
                    symbol, side: 'SHORT', entryPrice: parseFloat(realP.entryPrice), qty: Math.abs(parseFloat(realP.positionAmt)), 
                    tp: sync.tp, sl: sync.sl, margin: (Math.abs(parseFloat(realP.positionAmt)) * parseFloat(realP.entryPrice)) / info.maxLeverage,
                    leverage: info.maxLeverage, dcaCount: isDCA ? cp.dcaCount + 1 : 0, firstMargin: isDCA ? cp.firstMargin : marginToUse, isProcessing: false, pnl: 0,
                    tpslToggle: 15 // Mặc định để lần nhấn đầu tiên ra 10%
                });
            }
        }
    } catch (e) { addBotLog(`🚨 Lỗi: ${e.message}`, "error"); } finally { openingSymbols.delete(symbol); }
}
...
// API TEST CẬP NHẬT 5 CHỨC NĂNG
APP.post('/api/test', async (req, res) => {
    const { action, symbol } = req.body;
    const s = symbol.toUpperCase();
    const posKey = `${s}_SHORT`;
    const botPos = botActivePositions.get(posKey);
    const info = status.exchangeInfo[s];

    if (action === 'open') await openPosition(s, false, { margin: 0.3, tp: 10, sl: 50 });
    
    if (action === 'close') {
        if (botPos) await exchange.createOrder(s, 'MARKET', 'BUY', botPos.qty.toFixed(info.quantityPrecision), undefined, { positionSide: 'SHORT' });
    }
    
    if (action === 'clear_tpsl') {
        await clearAllOrders(s);
    }

    if (action === 'set_tpsl') {
        if (botPos) {
            botPos.tpslToggle = (botPos.tpslToggle === 10) ? 15 : 10;
            const tpPrice = (botPos.entryPrice * (1 - botPos.tpslToggle / 100)).toFixed(info.pricePrecision);
            const slPrice = (botPos.entryPrice * (1 + botSettings.posSL / 100)).toFixed(info.pricePrecision);
            await exchange.createOrder(s, 'TAKE_PROFIT_MARKET', 'BUY', undefined, undefined, { positionSide: 'SHORT', stopPrice: tpPrice, closePosition: true, workingType: 'MARK_PRICE' });
            await exchange.createOrder(s, 'STOP_MARKET', 'BUY', undefined, undefined, { positionSide: 'SHORT', stopPrice: slPrice, closePosition: true, workingType: 'MARK_PRICE' });
            addBotLog(`🎯 [${s}] Cài mới TP ${botPos.tpslToggle}% | SL ${botSettings.posSL}%`);
        }
    }

    if (action === 'dca') { // Nút Reset (Vừa xóa vừa cài)
        if (botPos) {
            await clearAllOrders(s);
            await new Promise(r => setTimeout(r, 1000));
            botPos.tpslToggle = (botPos.tpslToggle === 10) ? 15 : 10;
            ...
            await exchange.createOrder(s, 'TAKE_PROFIT_MARKET', 'BUY', undefined, undefined, { positionSide: 'SHORT', stopPrice: tpPrice, closePosition: true, workingType: 'MARK_PRICE' });
            await exchange.createOrder(s, 'STOP_MARKET', 'BUY', undefined, undefined, { positionSide: 'SHORT', stopPrice: slPrice, closePosition: true, workingType: 'MARK_PRICE' });
            addBotLog(`🔄 [${s}] Reset TP ${botPos.tpslToggle}% | SL ${botSettings.posSL}%`);
        }
    }
