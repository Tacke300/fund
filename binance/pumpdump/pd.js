...
// Helper lấy IPv4 để kiểm tra Whitelist
...
async function binancePrivate(endpoint, method = 'GET', data = {}) {
    ...
    try {
        ...
        // Nếu lỗi liên quan đến IP hoặc Auth, log chi tiết
        if (errorMsg.includes("IP") || errorMsg.includes("API-key")) {
            const myIP = getLocalIPv4();
            addBotLog(`❌ LỖI AUTH/IP: ${errorMsg} | IP Của Bạn: ${myIP}`, "error");
        }
...
async function openPosition(symbol, dcaData = null) {
    ...
    try {
        ...
        if (info.maxLeverage < 20) {
            addBotLog(`🚫 [${symbol}] MaxLev ${info.maxLeverage} < 20. Chặn.`, "warning");
        ...
        } else { marginToUse = dcaData.margin; }
        ...
        if (order) {
            ...
            if (realP) {
                ...
                botActivePositions.set(posKey, { 
                    symbol, side, entryPrice: entry, qty, tp: sync.tp, sl: sync.sl, margin: marginToUse,
                    ...
                    pnl: 0, markPrice: currentPrice
                });
                addBotLog(`✅ [${symbol}] ${side} ${dcaData?'DCA':'OPEN'} - Entry: ${entry}`, "success");
            }
        }
    ...
    try {
        ...
        for (let [key, botPos] of botActivePositions) {
            if (!exchangeKeys.has(key)) {
                ...
                if (!lastTrade) { openingSymbols.delete(botPos.symbol); continue; }
                ...
                if (realPnl > 0) {
                    addBotLog(`💰 [${botPos.symbol}] Chốt lời: ${realPnl.toFixed(2)} USDT`, "success");
                    status.blackList[botPos.symbol] = Date.now() + BLACKLIST_DURATION;
                    botActivePositions.delete(key);
                    openingSymbols.delete(botPos.symbol);
                } else {
                    addBotLog(`📉 [${botPos.symbol}] Dính SL. Đang DCA...`, "warning");
                    if (botPos.side === 'SHORT' && botPos.dcaCount < botSettings.maxDCA) {
                        const nextCount = botPos.dcaCount + 1;
                        const nextMargin = botPos.firstMargin * (nextCount + 1) * 1.1; 
                        botActivePositions.delete(key);
                        await openPosition(botPos.symbol, {
                            dcaCount: nextCount, margin: nextMargin, firstMargin: botPos.firstMargin,
                            historyEntries: botPos.historyEntries, prevAvgEntry: botPos.entryPrice,
                            prevQty: botPos.qty, isFinalLong: false
                        });
                    } else if (botPos.side === 'SHORT' && botPos.dcaCount >= botSettings.maxDCA) {
                        addBotLog(`🔥 [${botPos.symbol}] Đảo LONG x${FINAL_LONG_MULTIPLIate}`, "error");
                        botActivePositions.delete(key);
                        await openPosition(botPos.symbol, {
                            dcaCount: botPos.dcaCount + 1, margin: botPos.firstMargin * FINAL_LONG_MULTIPLIER,
                            firstMargin: botPos.firstMargin, historyEntries: botPos.historyEntries, isFinalLong: true
                        });
                    } else {
                        botActivePositions.delete(key);
                        openingSymbols.delete(botPos.symbol);
...
async function init() {
    addBotLog(`🔄 Đang khởi tạo... IP hiện tại: ${getLocalIPv4()}`);
    try {
        ...
        status.isReady = true;
        addBotLog(`😈 LUFFY V22.1 READY - IP: ${getLocalIPv4()}`, "success");
        priceMonitorLoop();
    } catch (e) { 
        addBotLog(`🚨 Khởi tạo thất bại: ${e.message}`, "error");
