...
async function loadExchangeInfo() {
    try {
        ...
        for (const [k, v] of Object.entries(markets)) {
            ...
            exchangeInfo[k] = {
                minCost: Math.max(v.limits?.cost?.min || 5.5, 5.5),
                minQty: v.limits?.amount?.min || 0,
                qtyPrecision: v.precision?.amount || 3,
                pricePrecision: v.precision?.price || 4
            };
        }
        marketReady = true;
...
async function preloadWallet() {
    try {
        ...
        walletCache = {
            totalWalletBalance: parseFloat(acc.info?.totalWalletBalance || 0).toFixed(2),
...
async function initWS() {
    try {
        ...
        const arr = res.data.filter(s => s.symbol.endsWith('USDT')).sort((a, b) => Math.abs(parseFloat(b.priceChangePercent)) - Math.abs(parseFloat(a.priceChangePercent))).slice(0, 120);
        arr.forEach(c => {
...
async function syncTPSL(pair, side, tp, sl) {
    try {
        ...
        for (const o of orders) {
            if (o.info.positionSide === side) await exchange.cancelOrder(o.id, pair);
        }
...
async function openPosition(symbol, side, price) {
    ...
    if (!info) return;
    let qty = (botSettings.capital * 20) / price;
    qty = Math.max(qty, info.minCost / price, info.minQty);
    qty = parseFloat(exchange.amountToPrecision(pair, qty));
    try {
        ...
        positions.set(key, { symbol, side, qty, leverage: 20, avg: price, tp, sl, pnl: 0, roi: 0, unrealized: 0, liquidationPrice: 0, markPrice: price, dca: 0, nextDca: side === 'LONG' ? price * (1 - botSettings.dcaPercent / 100) : price * (1 + botSettings.dcaPercent / 100) });
...
async function autoTradeLoop() {
    if (botSettings.isRunning) {
        for (const [s, v] of Object.entries(coinData)) {
            ...
            if (Math.abs(c1) >= botSettings.volVolatility || Math.abs(c5) >= botSettings.volVolatility || Math.abs(c15) >= botSettings.volVolatility) {
                const side = (c1 + c5 + c15) >= 0 ? 'LONG' : 'SHORT';
...
app.post('/api/closeall', async (req, res) => {
    for (const [key, p] of positions) { await closePosition(p); positions.delete(key); }
    res.json({ ok: true });
});
