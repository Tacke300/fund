...
// LOGIC LẤY MAX LEV (TRẢ LẠI Y HỆT BẢN CŨ CỦA M)
        ...
        https.get(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    logger("API Key lỗi hoặc không có quyền Futures, sử dụng fallback x20.", "ERR");
        ...
        https.get('https://fapi.binance.com/fapi/v1/exchangeInfo', (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    logger(`Fallback: Đã nạp ${allSymbols.length} coin (Mặc định x20).`, "INFO");
                } catch(e) {}
                resolve();
            });
        });
        ...
        tickers.forEach(t => {
            ...
            if (botState.running) {
                ...
                if (activePositions[t.s]) {
                    ...
                    if (Object.keys(activePositions).length <= 3) logger(`Bắt đầu lệnh: ${t.s} tại ${price}`, "INFO");
                }
            }
        });
    });
...
app.get('/api/data', (req, res) => {
    const activeData = Object.values(activePositions).map(p => {
        ...
        return { ...p, avgPrice, totalMargin, currentGrid: p.grids.length, roi: (pnl/totalMargin)*100, pnl, currentPrice: currentP };
    });
...
app.post('/api/reset', (req, res) => { 
    ...
    logger("RESET TOÀN BỘ", "ERR");
    saveState(); res.json({ status: 'ok' }); 
});

app.get('/gui', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Luffy Matrix v12</title><script src="https://cdn.tailwindcss.com"></script>
    <style>body{background:#0b0e11;color:#eaecef;font-family:monospace} th{cursor:pointer;background:#161a1e;padding:12px 8px;border-bottom:1px solid #2b3139} #logBox{background:#000;color:#0ecb81;padding:10px;height:220px;overflow-y:auto;font-size:11px;border:1px solid #333}</style>
        ...
        <div class="bg-[#1e2329] p-4 rounded-lg mb-4 border border-gray-800 grid grid-cols-2 md:grid-cols-7 gap-3">
            ...
            <div class="flex gap-1 items-end"><button onclick="sendCtrl(true)" class="bg-green-600 p-2 rounded font-bold flex-1">START</button><button onclick="sendCtrl(false)" class="bg-red-600 p-2 rounded font-bold flex-1">STOP</button></div>
        </div>
        ...
        <script>
            ...
            function render(){
                ...
                document.getElementById('activeBody').innerHTML = sorted.map(p=>\`<tr class="border-b border-gray-800">
                    <td class="p-2 font-bold text-yellow-500">\${p.symbol} <span class="text-gray-500 text-[10px]">\${p.maxLev}x</span></td>
