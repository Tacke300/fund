...
async function binancePrivate(endpoint, method = 'GET', data = {}, retryCount = 0) {
    try {
        ...
        if (e.response?.data?.code === -1021 && retryCount < 3) {
                ...
                if (currentLevel < pair.lastLevel) {
                    for (let k = pair.lastLevel - 1; k >= currentLevel; k--) {
                        if (!noteClosedThisTick && !pair.executedGridLevels[k]) {
                            ...
                            const newNote = { id: `Note_${Math.abs(k)}`, startLevel: k, gridMargin: pair.initialMargin, dcaNoteMargin: pair.initialMargin * 5, dcaNoteAvg: markP, dcaNoteCount: 1, executedDcaLevels: { [k]: true } };
...
setInterval(async () => {
    ...
    if (entrySignal) {
        ...
        try {
            ...
            addLog(`🚀 VÀO LỆNH MỚI | ${symbol} | Giá: ${formatPrice(startPrice)} | Vốn: ${gridMargin.toFixed(2)}$ mỗi chiều | Lev: x${info.maxLeverage}`, "open");
        } catch (e) {
