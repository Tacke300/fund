...
// --- HÀNG ĐỢI LỆNH (GIỮ NGUYÊN TỐC ĐỘ CỦA ÔNG) ---
let actionQueue = [];
...
// --- CLASS LUFFY CORE - GIỮ 100% LOGIC GỐC CỦA TÙNG ---
class LuffyCore {
    constructor(id, mode, minVol) {
        ...
        // Cấu hình gốc của ông
        this.tpTarget = 0.3; 
        this.dcaStep = 10.0; 
        this.maxDca = 100;
    }

    // Hàm tính toán ROI MARGIN chuẩn của ông (Không tính theo giá)
    getStats() {
        ...
        if (this.pending) {
            ...
            // TÍNH ROI MARGIN: (Giá hiện tại - Giá trung bình) / Giá trung bình * Đòn bẩy
    ...
    // LOGIC CẬP NHẬT BIẾN ĐỘNG - GIỮ NGUYÊN 100%
    update(s, p, c1, c5, c15) {
        ...
        if (this.pending) {
            ...
            // TP tính theo ROI Margin của ông
            const currentRoi = diffAvg * (h.type === 'LONG' ? 1 : -1) * (h.maxLev || 20);
            
            // Chốt lời khi ROI đạt target (Ví dụ 0.3% giá ~ 6% ROI với x20)
            const isWin = h.type === 'LONG' ? diffAvg >= this.tpTarget : diffAvg <= -this.tpTarget;

            if (isWin) {
                console.log(`[BOT #\${this.id+1}] CHỐT LÃI: \${s} | ROI: \${currentRoi.toFixed(2)}%`);
            ...
            // Logic DCA: 9 lần đầu mỗi 10%, sau đó mỗi 1%
            ...
            if (!actionQueue.find(q => q.id === `b\${this.id}`)) {
                actionQueue.push({ id: `b\${this.id}`, priority: 2, action: () => {
                    ...
                    console.log(`[BOT #\${this.id+1}] VÀO LỆNH: \${s} | TYPE: \${type} | VOL: \${this.minVol}%`);
                }});
            }
        }
    }
...
// --- GIAO DIỆN (CHIA 4 CỘT THEO YÊU CẦU) ---
...
app.get('/gui', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>LUFFY MATRIX 100% ORIGIN</title>
                ...
                group.forEach(b => {
                    ...
                    html += \`<div class="bot-card \${b.pending?'active':''}">
                        ...
                        <div class="grid grid-cols-2 gap-x-2 text-[10px] border-b border-zinc-800 pb-1 mb-1">
                            <div>Vốn: <b class="text-white">\${s.equity.toFixed(1)}</b></div>
                            <div>Lãi: <b class="text-green-400">\${s.pnlWin.toFixed(1)}</b></div>
                            <div>Khả dụng: <b class="text-yellow-500">\${s.available.toFixed(1)}</b></div>
                            <div class="text-right">Win: <b>\${s.winCount}</b></div>
                        </div>
                        <div class="flex justify-between items-center mt-1">
                            <div class="font-black \${s.livePnl>=0?'up':'down'}">\${s.livePnl>=0?'+':''}\${s.livePnl.toFixed(2)}$</div>
                            <div class="font-black \${s.roi>=0?'up':'down'}">\${s.roi.toFixed(1)}%</div>
                        </div>
                    </div>\`;
                });
