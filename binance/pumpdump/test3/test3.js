...
let sharedState = {
    ...
    exchangeInfo: null,
    closedHistory: [],
    initialBalance: null,  // Số dư gốc dùng để tính % lãi lỗ
    lastResetDay: null     // Ngày reset gần nhất để check mốc 7h sáng
};
...
async function closePositionAndLog(bot, b, markP, reasonStr) {
    try {
        ...
        if (reasonStr.includes("AVG")) {
            bot.status.avgCount++; bot.status.avgPnL += finalPnL;
        } else if (finalPnL >= 0) {
            bot.status.tpCount++; bot.status.tpPnL += finalPnL;
        } else {
            bot.status.slCount++; bot.status.slPnL += finalPnL;
        }
    ...
    try {
        ...
        for (let [key, b] of bot.botActivePositions) {
            ...
            if (realP) {
                ...
                let finalPnLFromSàn = matchingTrades.length > 0 ? trades.filter(t => t.positionSide === b.side && (nowServer - t.time) < 25000).reduce((sum, t) => sum + parseFloat(t.realizedPnl) - parseFloat(t.commission), 0) : b.pnl || 0;
                
                bot.status.botClosedCount++; bot.status.botPnLClosed += finalPnLFromSàn;
                let closedReason = "ĐÓNG TRÊN SÀN - SL";
                if (finalPnLFromSàn >= 0) {
                    bot.status.tpCount++; bot.status.tpPnL += finalPnLFromSàn; closedReason = "ĐÓNG TRÊN SÀN - TP";
                } else {
                    bot.status.slCount++; bot.status.slPnL += finalPnLFromSàn;
                }

                addBotLog(bot, `🔒 [ĐÓNG TRÊN SÀN] ${b.symbol} ${b.side} | Entry: ${b.avgEntry.toFixed(pPrec)} | PnL: ${finalPnLFromSàn.toFixed(2)}$`, finalPnLFromSàn >= 0 ? "success" : "sl");
...
async function openPosition(bot, symbol, dcaData = null, forcedSide = null, sharedQty = null, sharedMargin = null, sharedPrice = null, isDiangucSignal = false) {
    ...
    try {
        ...
        if (order) {
            ...
            let totalMargin = actualMarginUsed; let dcaHistory = [];
            ...
            addBotLog(bot, !isDCA ? `[MỞ ${side}][${currentModeIsHell ? "ĐỊA NGỤC" : "THƯỜNG"}] ${symbol} | Margin: ${totalMargin.toFixed(2)}$ | Entry: ${newAvgEntry.toFixed(info.pricePrecision)}` : `[DCA LẦN ${dcaCount}] ${symbol} | Avg Entry: ${newAvgEntry.toFixed(info.pricePrecision)}`, "open");
        }
...
// =========================================================
// KHỞI TẠO CẤU TRÚC SERVER & ROUTE
// =========================================================
...
appServer.get('/api/global_summary', async (req, res) => {
    ...
    // Đọc số dư thực tế từ tài khoản sàn (Bot 1 và 2 dùng chung tài khoản)
    ...
    // Xử lý logic Reset số dư mốc 7h sáng
    ...
    // Nếu chưa từng có số dư gốc (mới chạy bot), gán giá trị hiện tại làm gốc
    if (!sharedState.initialBalance || sharedState.initialBalance === 0) {
        sharedState.initialBalance = walletBalance;
    }

    // Tính phần trăm lãi lỗ thực tế dựa trên mốc gốc (+- %)
    let changePercent = 0;
...
appServer.post('/api/global_control', (req, res) => {
    ...
    // Nếu người dùng ấn START, ép hệ thống ghi nhận số dư gốc ngay tại thời điểm đó
    if (isRunning) {
        sharedState.initialBalance = null; 
    }
...
// GIAO DIỆN HTML DASHBOARD CHÍNH (PORT 2401)
    ...
    <body class="p-6">
        <div class="max-w-7xl mx-auto space-y-6">
            <header class="flex justify-between items-center pb-4 border-b border-gray-700">
                ...
                <!-- KHU VỰC THÔNG TIN TÀI KHOẢN VÀ BIẾN ĐỘNG % CHỮA CHÁY -->
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    <div class="card p-4 rounded shadow">
                        <div class="text-gray-400 text-xs font-medium uppercase">PnL Tổng Hợp (Lệnh Đã Đóng)</div>
                        <div id="statTotalPnL" class="text-2xl font-bold mt-1 text-white">0.00$</div>
                    </div>
                </div>
            </div>

            <!-- BẢNG DANH SÁCH VỊ THẾ ĐANG MỞ -->
        ...
        <script>
            ...
            async function updateDashboard() {
                try {
                    ...
                    // Cập nhật Khu Vực Tài Khoản thời gian thực
...
// Nhận dữ liệu từ bot quét biến động
setInterval(() => {
        ...
        res.on('end', () => { 
            try { 
                ...
                if (sharedState.candidatesList.length === 0) {
                    console.log("⚠️ [PORT 9000] Dữ liệu mảng 'live' rỗng. Không có coin biến động.");
                }
...
// =========================================================
// VÒNG LẶP ĐIỀU PHỐI ĐẶT LỆNH CHÍNH
// =========================================================
setInterval(async () => {
...
appServer.listen(2401, () => console.log('🌐 [MAIN SERVER] Lõi xử lý tổng hợp hoạt động tại Port 2401'));
appBot1.listen(2402, () => console.log('📈 [BOT 1 UI] Web theo dõi hoạt động tại Port 2402'));
appBot2.listen(2403, () => console.log('📉 [BOT 2 UI] Web theo dõi hoạt động tại Port 2403'));
