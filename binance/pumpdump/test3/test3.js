...
// =========================================================
// BỘ NHỚ CHIA SẺ & THÔNG SỐ SERVER
// =========================================================
let sharedState = {
    blackList: {},
    permanentBlacklist: {},
...
// =========================================================
// CẤU TRÚC 2 BOT INSTANCE
// =========================================================
let bot1 = {
    id: "BOT_1", sideMode: "NORMAL", 
    botSettings: { isRunning: false, maxPositions: 3, invValue: "1%", minVol: 7, posTP: 10, posSL: 10.0, dianguctp: 30, diangucsl: 10, diangucdca: 10, posdca: 3, diangucvol: 15, maxDCA: MAX_DCA_LEVEL, heSoThuong: 2, heSoDianguc: 3 },
    status: { botLogs: [], botClosedCount: 0, botPnLClosed: 0, isReady: false, totalOpenedCount: 0, tpCount: 0, tpPnL: 0, avgCount: 0, avgPnL: 0, slCount: 0, slPnL: 0 },
...
function checkEntryCondition(candidate, botSettings, status, botActivePositions) {
    if (status.blackList[candidate.symbol] || status.permanentBlacklist[candidate.symbol]) return null;
    if (botActivePositions.has(`${candidate.symbol}_SHORT`) || botActivePositions.has(`${candidate.symbol}_LONG`)) return null;
    ...
    for (const tf of SCAN_CONFIG.DIA_NGUC) {
        if (timeframes[tf] !== undefined && Math.abs(timeframes[tf]) >= diangucVol) 
            return { symbol: candidate.symbol, side: timeframes[tf] > 0 ? 'LONG' : 'SHORT', vol: Math.abs(timeframes[tf]), reason: tf, isDianguc: true };
    }
    for (const tf of SCAN_CONFIG.THUONG) {
        if (timeframes[tf] !== undefined && Math.abs(timeframes[tf]) >= minVol) 
            return { symbol: candidate.symbol, side: timeframes[tf] > 0 ? 'LONG' : 'SHORT', vol: Math.abs(timeframes[tf]), reason: tf, isDianguc: false };
    }
    return null;
}
...
function checkAndAddBlacklist(symbol) {
    ...
    if (!hasBot1 && !hasBot2) {
        ...
        addBotLog(bot1, `🚫 [BLACKLIST] Đã chặn ${symbol} 15 phút.`, "warn"); addBotLog(bot2, `🚫 [BLACKLIST] Đã chặn ${symbol} 15 phút.`, "warn");
    }
}
...
function pushToClosedHistory(botId, position, closePrice, finalPnL, reasonStr) {
    ...
    sharedState.closedHistory.unshift({
        time, botId, symbol: position.symbol, side: position.side, dcaCount: position.dcaCount,
        isDiangucMode: position.isDiangucMode, margin: position.currentMargin, qty: position.currentQty,
        avgEntry: position.avgEntry, closePrice: closePrice, pnl: finalPnL, profitPercent: position.profitPercent, reason: reasonStr
    });
    if (sharedState.closedHistory.length > 100) sharedState.closedHistory.pop();
}

async function closePositionAndLog(bot, b, markP, reasonStr) {
    try {
        const info = sharedState.exchangeInfo[b.symbol]; const pPrec = info ? info.pricePrecision : 6; 
        ...
        let finalPnL = 0;
        if (matchingTrades.length > 0) finalPnL = matchingTrades.reduce((sum, t) => sum + parseFloat(t.realizedPnl) - parseFloat(t.commission), 0);
        else finalPnL = (b.side === 'LONG' ? (markP - b.avgEntry) * b.currentQty : (b.avgEntry - markP) * b.currentQty) - (b.currentQty * markP * 0.001);

        bot.status.botClosedCount++; bot.status.botPnLClosed += finalPnL;
        if (reasonStr.includes("AVG")) { bot.status.avgCount++; bot.status.avgPnL += finalPnL; } 
        else if (finalPnL >= 0) { bot.status.tpCount++; bot.status.tpPnL += finalPnL; } 
        else { bot.status.slCount++; bot.status.slPnL += finalPnL; }

        let logType = finalPnL >= 0 ? "success" : "sl"; if (reasonStr.includes("AVG")) logType = "avg"; 
        addBotLog(bot, `🔒 [${reasonStr}] ${b.symbol} ${b.side} | PnL: ${finalPnL.toFixed(2)}$`, logType);
    ...
    } catch (e) { addBotLog(bot, `❌ Lỗi đóng ${b.symbol}: ${e.message}`, "error"); }
}
...
async function panicCloseAll(bot, reasonLog) {
    try {
        ...
        for (const p of active) {
            const side = p.positionSide; const qty = Math.abs(parseFloat(p.positionAmt));
            try {
                const key = `${p.symbol}_${side}`; const cachedPos = bot.botActivePositions.get(key);
                await bot.exchange.createOrder(p.symbol, 'MARKET', side === 'SHORT' ? 'BUY' : 'SELL', qty, undefined, { positionSide: side });
                if (cachedPos) pushToClosedHistory(bot.id, cachedPos, cachedPos.livePrice, cachedPos.pnl || 0, "PANIC CLOSE");
                count++;
            } catch (err) { }
        }
...
async function priceMonitor(bot) {
    ...
    try {
        ...
        for (let [key, b] of bot.botActivePositions) {
            ...
            const lockKey = `${b.symbol}_${b.side}`; const info = sharedState.exchangeInfo[b.symbol];
            const pPrec = info ? info.pricePrecision : 6; 

            if (realP) {
                const currentQty = Math.abs(parseFloat(realP.positionAmt)); const markP = parseFloat(realP.markPrice); const avgEntry = parseFloat(realP.entryPrice); 
                ...
                // 💥 KIỂM TRA ƯU TIÊN CHẾ ĐỘ ĐỊA NGỤC (ĐÓNG VỊ THẾ THƯỜNG NẾU ĐẠT VOL ĐỊA NGỤC)
                if (!b.isDiangucMode) {
                    const cand = sharedState.candidatesList.find(c => c.symbol === b.symbol);
                    if (cand) {
                        const hellVol = parseFloat(bot.botSettings.diangucvol);
                        if (Math.abs(parseFloat(cand.c1)) >= hellVol || Math.abs(parseFloat(cand.c5)) >= hellVol || Math.abs(parseFloat(cand.c15)) >= hellVol) {
                            bot.botActivePositions.delete(key);
                            delete sharedState.blackList[b.symbol]; // Xóa chặn để mở lại địa ngục
                ...
                if (shouldCloseMarket) {
                    bot.botActivePositions.delete(key); await closePositionAndLog(bot, b, markP, "CHỐT TRAILING AVG"); checkAndAddBlacklist(b.symbol); continue;
                }
                ...
                const trades = await binancePrivate(bot, '/fapi/v1/userTrades', 'GET', { symbol: b.symbol, limit: 12 }).catch(() => []);
                const nowServer = Date.now() + bot.timestampOffset;
                let finalPnLFromSàn = trades.length > 0 ? trades.filter(t => t.positionSide === b.side && (nowServer - t.time) < 25000).reduce((sum, t) => sum + parseFloat(t.realizedPnl) - parseFloat(t.commission), 0) : b.pnl || 0;
                
                bot.status.botClosedCount++; bot.status.botPnLClosed += finalPnLFromSàn;
                let closedReason = "ĐÓNG TRÊN SÀN - SL";
                if (finalPnLFromSàn >= 0) { bot.status.tpCount++; bot.status.tpPnL += finalPnLFromSàn; closedReason = "ĐÓNG TRÊN SÀN - TP"; } 
                else { bot.status.slCount++; bot.status.slPnL += finalPnLFromSàn; }

                addBotLog(bot, `🔒 [ĐÓNG TRÊN SÀN - TP/SL] ${b.symbol} ${b.side} | PnL: ${finalPnLFromSàn.toFixed(2)}$`, finalPnLFromSàn >= 0 ? "success" : "sl");
                pushToClosedHistory(bot.id, b, b.livePrice, finalPnLFromSàn, closedReason);
                bot.botActivePositions.delete(key); checkAndAddBlacklist(b.symbol); 
            }
        }
    } catch (e) { }
...
async function openPosition(bot, symbol, dcaData = null, forcedSide = null, sharedQty = null, sharedMargin = null, sharedPrice = null, isDiangucSignal = false) {
    ...
    try {
        ...
        const pPrec = info.pricePrecision; let qty = 0, margin = 0, currentPrice = 0;
        ...
        if (order) {
            ...
            let actualMarginUsed = (qty * actualFilledPrice) / info.maxLeverage; let totalMargin = actualMarginUsed; let dcaHistory = [];

            if (isDCA) {
                const oldQty = dcaData.currentQty; const oldAvg = dcaData.avgEntry; totalQty = oldQty + qty;
                ...
                bot.status.totalOpenedCount++; dcaHistory = [{ price: actualFilledPrice, margin: actualMarginUsed }];
            }
            ...
            if (!isDCA) {
                ...
                finalTP = sync.tp; finalSL = sync.sl;
            ...
            bot.botActivePositions.set(lockKey, { 
                symbol, side, entryPrice: firstE, tp: finalTP, sl: finalSL, dcaCount: dcaCount, leverage: info.maxLeverage, firstEntry: firstE, firstMargin: dcaData ? dcaData.firstMargin : actualMarginUsed, 
                currentMargin: totalMargin, currentQty: totalQty, dcaHistory: dcaHistory, isDiangucMode: currentModeIsHell, pnl: 0, profitPercent: 0, avgEntry: newAvgEntry, 
                nextDCA: side === 'LONG' ? firstE * (1 + ((dcaCount + 1) * (dcaThreshold / 100))) : firstE * (1 - ((dcaCount + 1) * (dcaThreshold / 100))), livePrice: actualFilledPrice
            });
            
            addBotLog(bot, !isDCA ? `[MỞ ${side}][${currentModeIsHell ? "ĐỊA NGỤC" : "THƯỜNG"}] ${symbol} | M: ${totalMargin.toFixed(2)}$ | E: ${newAvgEntry.toFixed(pPrec)}` : `[DCA ${dcaCount}] ${symbol} | Avg: ${newAvgEntry.toFixed(pPrec)}`, "open");
        }
    } catch (e) { sharedState.permanentBlacklist[symbol] = true; addBotLog(bot, `❌ [BAN VĨNH VIỄN] Lỗi tại ${symbol}: ${e.message}`, "error"); }
    ...
    if (acc) {
        ...
        if (totalWallet > 0) {
            ...
            if (availPercent <= ANTI_LIQUIDATION_LIMIT) {
                await panicCloseAll(bot, "CHỐT BẢO VỆ 5%"); bot.isMarginProtected = true; bot.botSettings.isRunning = false; return; 
            }
...
// =========================================================
// KHỞI TẠO CẤU TRÚC SERVER & ROUTE (CHÍNH XÁC NHƯ YÊU CẦU)
// =========================================================
...
// PHỤC VỤ FILE INDEX.HTML CỦA BẠN TỪ THƯ MỤC GỐC CHO BOT 1 & 2
appBot1.use(express.static(__dirname));
appBot2.use(express.static(__dirname));
...
async function buildStatusResponse(bot) {
    ...
    for (const [sym, expireTime] of Object.entries(sharedState.blackList)) {
        const rem = Math.floor((expireTime - now) / 1000); if (rem > 0) formattedBlacklist[sym] = rem;
    }
    return { 
        ...
        status: { botLogs: bot.status.botLogs, botClosedCount: bot.status.botClosedCount, botPnLClosed: bot.status.botPnLClosed, isReady: bot.status.isReady, candidatesList: sharedState.candidatesList, blackList: formattedBlacklist, permanentBlacklist: sharedState.permanentBlacklist, totalOpenedCount: bot.status.totalOpenedCount, tpCount: bot.status.tpCount, tpPnL: bot.status.tpPnL, avgCount: bot.status.avgCount, avgPnL: bot.status.avgPnL, slCount: bot.status.slCount, slPnL: bot.status.slPnL }, 
        wallet: acc ? { totalWalletBalance: parseFloat(acc.totalMarginBalance || 0), availableBalance: parseFloat(acc.availableBalance || 0), totalUnrealizedProfit: parseFloat(acc.totalUnrealizedProfit || 0) } : { totalWalletBalance: 0, availableBalance: 0, totalUnrealizedProfit: 0 } 
    };
}
...
appBot1.post('/api/close_position', async (req, res) => {
    ...
    if (b) { try { await closePositionAndLog(bot1, b, b.livePrice, "ĐÓNG THỦ CÔNG"); bot1.botActivePositions.delete(key); checkAndAddBlacklist(symbol); return res.json({ success: true }); } catch (e) { return res.json({ success: false, msg: e.message }); } }
...
appBot2.post('/api/close_position', async (req, res) => {
    ...
    if (b) { try { await closePositionAndLog(bot2, b, b.livePrice, "ĐÓNG THỦ CÔNG"); bot2.botActivePositions.delete(key); checkAndAddBlacklist(symbol); return res.json({ success: true }); } catch (e) { return res.json({ success: false, msg: e.message }); } }
    res.json({ success: false });
});

// =========================================================
// GIAO DIỆN & API SERVER TỔNG (PORT 2401)
// =========================================================
appServer.get('/api/global_summary', async (req, res) => {
    ...
    const walletBalance = data1.wallet.totalWalletBalance;
    const availableBalance = data1.wallet.availableBalance;
    const unrealizedProfit = data1.wallet.totalUnrealizedProfit;

    const nowTime = new Date();
    const currentDay = nowTime.getDate();
    if (nowTime.getHours() >= 7 && sharedState.lastResetDay !== currentDay) {
        sharedState.initialBalance = walletBalance;
        sharedState.lastResetDay = currentDay;
        addBotLog(bot1, `🌅 [HỆ THỐNG] Đã reset tự động mốc số dư lúc 7h sáng: ${walletBalance}$`, "info");
    }

    if (!sharedState.initialBalance || sharedState.initialBalance === 0) {
        sharedState.initialBalance = walletBalance;
    }

    let changePercent = 0;
    if (sharedState.initialBalance > 0) {
        changePercent = ((walletBalance - sharedState.initialBalance) / sharedState.initialBalance) * 100;
    }

    const combinedPositions = [...data1.activePositions.map(p => ({...p, botId: 'BOT 1'})), ...data2.activePositions.map(p => ({...p, botId: 'BOT 2'}))];
    res.json({
        systemRunning: bot1.botSettings.isRunning && bot2.botSettings.isRunning,
        account: {
            walletBalance: walletBalance.toFixed(2),
            availableBalance: availableBalance.toFixed(2),
            unrealizedProfit: unrealizedProfit.toFixed(2),
            changePercent: changePercent.toFixed(2),
            initialBalance: sharedState.initialBalance.toFixed(2)
        },
        stats: {
        ...
        closedHistory: sharedState.closedHistory,
        bot1Logs: data1.status.botLogs,
        bot2Logs: data2.status.botLogs,
        configs: { bot1: data1.botSettings, bot2: data2.botSettings }
    });
});
...
appServer.post('/api/global_control', (req, res) => {
    ...
    if (isRunning) sharedState.initialBalance = null; 
    
    addBotLog(bot1, `🌐 [SERVER] Trạng thái cập nhật: ${action.toUpperCase()}`, "warn");
    addBotLog(bot2, `🌐 [SERVER] Trạng thái cập nhật: ${action.toUpperCase()}`, "warn");
    res.json({ success: true, isRunning });
});

// HTML ĐƯỢC NHÚNG TRỰC TIẾP CHO SERVER 2401
const MAIN_SERVER_HTML = `
<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .log-open { color: #38bdf8; } .log-dca { color: #c084fc; } .log-info { color: #cbd5e1; }
    </style>
</head>
<body class="p-6">
    <div class="max-w-7xl mx-auto space-y-6">
        <header class="flex justify-between items-center pb-4 border-b border-gray-700">
            <div>
                <h1 class="text-3xl font-bold text-blue-400">MAIN SERVER PORT 2401</h1>
                <p class="text-gray-400 text-sm">Quản lý tổng hợp thời gian thực</p>
            </div>
            <div class="flex items-center space-x-4 bg-slate-800 p-3 rounded-lg border border-slate-700 shadow-lg">
                <div class="text-right border-r border-slate-600 pr-4">
                    <div class="text-gray-400 text-xs font-semibold">SỐ DƯ VÍ</div>
                    <div id="lblWalletBalance" class="text-lg font-bold text-green-400">0.00$</div>
                </div>
                <div class="text-right border-r border-slate-600 pr-4">
                    <div class="text-gray-400 text-xs font-semibold">KHẢ DỤNG</div>
                    <div id="lblAvailableBalance" class="text-lg font-bold text-blue-400">0.00$</div>
                </div>
                <div class="text-right pr-2">
                    <div class="text-gray-400 text-xs font-semibold">HIỆU SUẤT TÀI KHOẢN</div>
                    <div id="lblWalletPnLPercent" class="text-lg font-bold text-white">0.00%</div>
                </div>
                <div class="pl-2 border-l border-slate-600">
                    <span id="sysStatus" class="px-3 py-1.5 rounded font-bold text-xs text-white bg-gray-600 block text-center">ĐANG TẢI...</span>
                    <button id="btnToggle" onclick="toggleSystem()" class="mt-1 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-3 py-1 rounded transition shadow-md block w-full">ĐỒNG BỘ</button>
                </div>
            </div>
        </header>
                ...
                <table class="w-full text-left text-sm text-gray-300">
                    <thead>
                        <tr class="border-b border-gray-700 text-gray-400 font-medium">
                            <th class="pb-2">BOT</th><th class="pb-2">Cặp Coin</th><th class="pb-2">Vị Thế</th><th class="pb-2">Số Lần DCA</th>
                            <th class="pb-2">Margin</th><th class="pb-2">Kích Thước</th><th class="pb-2">Entry Đầu / Avg</th><th class="pb-2">Giá Live</th>
                            <th class="pb-2">Mục Tiêu TP / SL</th><th class="pb-2">PnL (%)</th>
                        </tr>
                    </thead>
                    <tbody id="positionTable" class="divide-y divide-gray-800">
                        <tr><td colspan="10" class="py-4 text-center text-gray-500">Không có vị thế hoạt động.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="card p-6 rounded shadow">
                <h3 class="text-lg font-bold text-blue-400 border-b border-gray-700 pb-2 mb-4">⚙️ CẤU HÌNH BOT 1 (PORT 2402)</h3>
                <pre id="configBot1" class="text-xs text-green-400 overflow-x-auto p-3 bg-black rounded h-40">Đang tải...</pre>
            </div>
            <div class="card p-6 rounded shadow">
                <h3 class="text-lg font-bold text-pink-400 border-b border-gray-700 pb-2 mb-4">⚙️ CẤU HÌNH BOT 2 (PORT 2403)</h3>
                <pre id="configBot2" class="text-xs text-green-400 overflow-x-auto p-3 bg-black rounded h-40">Đang tải...</pre>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="card p-4 rounded shadow">
                <h3 class="text-sm font-bold text-blue-400 mb-2">📟 LOG REAL-TIME BOT 1</h3>
                <div id="logBot1" class="log-container p-2 rounded"></div>
            </div>
            <div class="card p-4 rounded shadow">
                <h3 class="text-sm font-bold text-pink-400 mb-2">📟 LOG REAL-TIME BOT 2</h3>
        ...
        <div class="card p-6 rounded shadow">
            <h3 class="text-lg font-bold text-green-400 mb-4 flex items-center">
                ...
                📜 LỊCH SỬ VỊ THẾ ĐÃ ĐÓNG (100 LỆNH GẦN NHẤT)
            </h3>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-300">
                    <thead>
                        <tr class="border-b border-gray-700 text-gray-400 font-medium">
                            <th class="pb-2">Thời Gian</th><th class="pb-2">BOT</th><th class="pb-2">Cặp Coin</th><th class="pb-2">Vị Thế</th>
                            <th class="pb-2">DCA</th><th class="pb-2">Margin</th><th class="pb-2">Kích Thước</th><th class="pb-2">Avg Entry</th>
                            <th class="pb-2">Giá Đóng</th><th class="pb-2">Lý Do</th><th class="pb-2">PnL</th>
                        </tr>
                    </thead>
                    <tbody id="historyTable" class="divide-y divide-gray-800">
                        <tr><td colspan="11" class="py-4 text-center text-gray-500">Chưa có lịch sử lệnh đóng.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <script>
        ...
        function parseLogClass(type) {
            if (type === 'success') return 'log-success'; if (type === 'error' || type === 'sl') return 'log-error';
            if (type === 'warn') return 'log-warn'; if (type === 'open') return 'log-open'; if (type === 'dca') return 'log-dca';
            return 'log-info';
        }

        async function updateDashboard() {
            try {
                const res = await fetch('/api/global_summary'); const data = await res.json();
                currentStatus = data.systemRunning;
                
                const statusEl = document.getElementById('sysStatus'); const btnToggle = document.getElementById('btnToggle');
                if(currentStatus) {
                    statusEl.innerText = "HỆ THỐNG: START"; statusEl.className = "px-3 py-1.5 rounded font-bold text-xs text-white bg-green-600 block text-center";
                    btnToggle.innerText = "STOP SYSTEM"; btnToggle.className = "mt-1 bg-red-600 hover:bg-red-700 text-white font-bold text-xs px-3 py-1 rounded transition shadow-md block w-full";
                } else {
                    statusEl.innerText = "HỆ THỐNG: STOP"; statusEl.className = "px-3 py-1.5 rounded font-bold text-xs text-white bg-red-600 block text-center";
                    btnToggle.innerText = "START SYSTEM"; btnToggle.className = "mt-1 bg-green-600 hover:bg-green-700 text-white font-bold text-xs px-3 py-1 rounded transition shadow-md block w-full";
                }

                document.getElementById('lblWalletBalance').innerText = data.account.walletBalance + "$";
                document.getElementById('lblAvailableBalance').innerText = data.account.availableBalance + "$";
                
                const pnlPercent = parseFloat(data.account.changePercent);
                const pnlPercentEl = document.getElementById('lblWalletPnLPercent');
                ...
                const totalPnL = data.stats.totalPnL; const pnlEl = document.getElementById('statTotalPnL');
                pnlEl.innerText = totalPnL.toFixed(2) + "$"; pnlEl.className = "text-2xl font-bold mt-1 " + (totalPnL >= 0 ? "text-green-400" : "text-red-400");
                ...
                if(data.activePositions.length === 0) {
                    ...
                    tbody.innerHTML = data.activePositions.map(p => {
                        return '<tr class="border-b border-gray-800 hover:bg-gray-800 transition">' +
                            '<td class="py-3 font-semibold text-gray-400">' + p.botId + '</td><td class="py-3 font-bold text-white">' + p.symbol + '</td>' +
                            '<td class="py-3 ' + (p.side === 'LONG' ? 'text-green-400' : 'text-red-400') + ' font-bold">' + p.side + '</td><td>' + p.dcaCount + ' / ' + (p.isDiangucMode ? 'ĐN' : 'T') + '</td>' +
                            '<td class="py-3 font-medium">' + p.currentMargin.toFixed(2) + '$</td><td>' + p.currentQty + '</td>' +
                            '<td class="py-3 text-xs">' + p.firstEntry.toFixed(4) + ' / <span class="text-blue-300 font-bold">' + p.avgEntry.toFixed(4) + '</span></td>' +
                            '<td class="py-3 font-bold">' + p.livePrice.toFixed(4) + '</td><td class="py-3 text-xs text-gray-400">TP: <span class="text-green-400">' + p.tp.toFixed(4) + '</span><br>SL: <span class="text-red-400">' + p.sl.toFixed(4) + '</span></td>' +
                            '<td class="py-3 ' + (p.pnl >= 0 ? 'text-green-400' : 'text-red-400') + ' font-bold">' + p.pnl.toFixed(2) + '$ (' + p.profitPercent.toFixed(2) + '%)</td>' +
                        '</tr>';
                    }).join('');
                }
                ...
                if(!data.closedHistory || data.closedHistory.length === 0) {
                    ...
                    hbody.innerHTML = data.closedHistory.map(h => {
                        return '<tr class="border-b border-gray-800 hover:bg-gray-800/50 transition">' +
                            '<td class="py-2 text-xs text-gray-400">' + h.time + '</td><td class="py-2 text-xs font-semibold text-gray-500">' + h.botId + '</td>' +
                            '<td class="py-2 font-bold text-white">' + h.symbol + '</td><td class="py-2 ' + (h.side === 'LONG' ? 'text-green-400' : 'text-red-400') + ' font-bold text-xs">' + h.side + '</td>' +
                            '<td class="py-2 text-xs">' + h.dcaCount + ' (' + (h.isDiangucMode ? 'ĐN' : 'T') + ')</td><td class="py-2 text-xs">' + h.margin.toFixed(2) + '$</td>' +
                            '<td class="py-2 text-xs">' + h.qty + '</td><td class="py-2 text-xs">' + h.avgEntry.toFixed(4) + '</td><td class="py-2 text-xs font-bold">' + h.closePrice.toFixed(4) + '</td>' +
                            '<td class="py-2 text-xs font-medium text-purple-400">' + h.reason + '</td><td class="py-2 ' + (h.pnl >= 0 ? 'text-green-400' : 'text-red-400') + ' font-bold text-xs">' + h.pnl.toFixed(2) + '$</td>' +
                        '</tr>';
                    }).join('');
                }
            } catch (e) { }
        ...
        async function toggleSystem() {
            ...
            if(confirm("Xác nhận " + action.toUpperCase() + " toàn bộ hệ thống?")) {
                await fetch('/api/global_control', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action }) });
                updateDashboard();
            }
        }
        setInterval(updateDashboard, 1500); updateDashboard();
    </script>
</body>
</html>
`;
...
async function init() {
    try {
        ...
        console.log(`🚀 [SUCCESS] Kết nối Binance thành công. Sẵn sàng nhận lệnh.`);
    } catch (e) { 
        console.error("❌ [LỖI KẾT NỐI BINANCE]: Thử lại sau 5s...", e.message);
    ...
    http.get('http://127.0.0.1:9000/api/data', res => {
        ...
        res.on('end', () => { 
            try { 
                ...
                sharedState.candidatesList = parsed.live || []; 
                if (sharedState.candidatesList.length === 0) {
                    // console.log("⚠️ [PORT 9000] Dữ liệu mảng 'live' rỗng.");
                }
            } catch(e){ console.error("❌ Lỗi parse JSON nhận từ port 9000"); } 
        });
    }).on('error', () => { console.error("❌ Không thể kết nối tới Port 9000 (Bot quét)"); });
}, 1500);
...
setInterval(async () => {
    ...
    if (bot1.botActivePositions.size < bot1.botSettings.maxPositions && 
        ...
        if (entrySignal) {
            ...
            openPosition(bot1, symbol, null, bot1.sideMode === 'REVERSED' ? (entrySignal.side === 'LONG' ? 'SHORT' : 'LONG') : entrySignal.side, finalQty, finalMargin, currentPrice, entrySignal.isDianguc);
            openPosition(bot2, symbol, null, bot2.sideMode === 'REVERSED' ? (entrySignal.side === 'LONG' ? 'SHORT' : 'LONG') : entrySignal.side, finalQty, finalMargin, currentPrice, entrySignal.isDianguc);
        }
    }
}, 3000); 

appServer.listen(2401, () => console.log('🌐 [MAIN SERVER] Web Quản Trị Server hoạt động tại: http://localhost:2401'));
appBot1.listen(2402, () => console.log('📈 [BOT 1] Đang mở API và load index.html tĩnh tại: http://localhost:2402'));
appBot2.listen(2403, () => console.log('📉 [BOT 2] Đang mở API và load index.html tĩnh tại: http://localhost:2403'));
