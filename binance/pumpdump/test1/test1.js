...
function initWS() {
    ...
    ws.on('message', (data) => {
        ...
        tickers.forEach(t => {
            ...
            if (!botConfig.running) return; // Nếu bot đang STOP thì chỉ cập nhật giá, không trade
            ...
            if (pending) {
                ...
                // DCA (Luôn chạy bất kể 50% margin)
...
app.get('/api/config', (req, res) => {
    ...
    botConfig.tp = parseFloat(botConfig.tp);
    botConfig.sl = parseFloat(botConfig.sl);
    botConfig.vol = parseFloat(botConfig.vol);
...
app.get('/gui', (req, res) => {
    ...
    <div class="p-4 bg-[#0b0e11] sticky top-0 z-50 border-b border-zinc-800">
        <div id="setupPanel" class="grid grid-cols-2 gap-3 mb-4 bg-card p-3 rounded-lg">
            ...
            <button id="btnAction" onclick="toggleBot()" class="col-span-2 bg-[#fcd535] hover:bg-[#ffe066] text-black py-2.5 rounded-md font-bold uppercase text-xs mt-2"></button>
        </div>
        ...
        <div class="flex justify-between items-start mb-4">
            ...
            <div class="text-right text-[10px] uppercase font-bold">
                <div class="text-green-500">WIN: <span id="winCount">0</span> | PNL: <span id="winPnl">0.00</span></div>
                <div id="botStatusBadge" class="mt-1 px-2 py-0.5 rounded inline-block text-black"></div>
            </div>
        </div>
    ...
    <div class="px-4 mt-5"><div class="bg-card rounded-xl p-4 shadow-lg">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[10px] text-left"><thead class="text-gray-custom uppercase border-b border-zinc-800"><tr><th>STT</th><th>Pair</th><th>DCA</th><th>Margin</th><th class="text-center">Lev/Target</th><th>Entry/Live</th><th>Avg Price</th><th class="text-right">PnL (ROI%)</th></tr></thead><tbody id="pendingBody"></tbody></table></div>
    </div></div>
    ...
    <script>
    let myChart = null, isStoppedByMarginLocal = false, isRunningLocal = false;
    ...
    function toggleBot() {
        const isCurrentlyRunning = isRunningLocal;
    ...
    async function update() {
        try {
            ...
            if(isRunningLocal) {
                btn.innerText = "STOP ENGINE (Giữ cấu hình)"; btn.className = "col-span-2 bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-md font-bold uppercase text-xs mt-2";
                badge.innerText = "RUNNING"; badge.className = "mt-1 px-2 py-0.5 rounded inline-block text-black bg-green-500 font-bold";
            } else {
                btn.innerText = "START ENGINE"; btn.className = "col-span-2 bg-[#fcd535] hover:bg-[#ffe066] text-black py-2.5 rounded-md font-bold uppercase text-xs mt-2";
                badge.innerText = "STOPPED"; badge.className = "mt-1 px-2 py-0.5 rounded inline-block text-black bg-gray-500 font-bold";
            }

            // Điền lại Input nếu chưa có (Yêu cầu 3)
            if(!document.getElementById('balanceInp').value) {
            ...
            // Dòng thông số cấu hình (Yêu cầu mới)
            ...
            let histHTML = [...d.history].sort((a,b)=>a.endTime-b.endTime).map((h, idx) => {
                ...
                return \`<tr class="border-b border-zinc-800/30"><td>\${idx+1}</td><td>\${new Date(h.endTime).toLocaleTimeString([],{hour12:false})}</td><td><b class="text-white">\${h.symbol}</b> <span class="\${h.type==='LONG'?'up':'down'}">\${h.type}</span></td><td class="text-yellow-500 font-bold">\${h.dcaCount}</td><td>\${totalMargin.toFixed(1)}</td><td>\${h.maxLev}x</td><td>\${fPrice(h.snapPrice)}<br>\${fPrice(h.finalPrice)}</td><td class="text-center down">\${h.maxNegativeRoi.toFixed(1)}%</td><td class="\${pnl>=0?'up':'down'} font-bold">\${pnl.toFixed(2)}</td><td class="text-right">\${runningBal.toFixed(1)}</td></tr>\`;
            }).reverse().join('');

            let avail = runningBal; 
            ...
            document.getElementById('stopLogBody').innerHTML = d.stopLogs.map(l => \`
                <div class="p-2 border-l-2 border-red-500 bg-red-500/5 mb-1">
                    ...
                    Win: \${l.data.winPnl.toFixed(1)} | Margin: \${l.data.totalMargin.toFixed(1)} | Avail: \${l.data.avail.toFixed(1)}
                </div>\`).join('');
