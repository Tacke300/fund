...
app.get('/gui', (req, res) => {
    ...
    <style>
        ...
        .recovery-row { background-color: rgba(147, 51, 234, 0.2) !important; color: #d8b4fe !important; border-left: 3px solid #a855f7; }
    </style></head><body>
    ...
    <div class="p-4 bg-[#0b0e11] sticky top-0 z-50 border-b border-zinc-800">
        ...
        <div class="flex justify-between items-end mb-3">
            <div>
                <div class="text-gray-500 text-[11px] uppercase font-bold tracking-widest">Equity</div>
                <span id="displayBal" class="text-4xl font-bold text-white tracking-tighter">0.00</span>
                <div class="text-[11px] text-blue-400 font-bold uppercase mt-1">Available: <span id="displayAvail">0.00</span></div>
            </div>
            <div class="text-right"><div class="text-gray-500 text-[11px] uppercase font-bold">PnL Tạm tính</div><div id="unPnl" class="text-xl font-bold">0.00</div></div>
        </div>
    </div>
    ...
    <div class="px-4 mt-5 mb-10"><div class="bg-card rounded-xl p-4 shadow-lg">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[9px] text-left"><thead class="text-gray-500 border-b border-zinc-800 uppercase"><tr><th>Time Out</th><th>Pair</th><th>DCA</th><th>Margin</th><th>Entry/Out</th><th>Vol Snap</th><th class="text-right">Available</th></tr></thead><tbody id="historyBody"></tbody></table></div>
    </div></div>
    ...
    if(saved.initialBal) {
        ...
        if(saved.running) { running = true; document.getElementById('setup').classList.add('hidden'); document.getElementById('active').classList.remove('hidden'); fetch(\`/api/config?tp=\${saved.tp}&sl=\${saved.sl}&vol=\${saved.vol}&mode=\${saved.mode}\`); }
    }
    ...
    async function update() {
        try {
            ...
            let historyTemp = [...d.history].reverse().map((h) => {
                ...
                return { ...h, totalMargin, availSnap: runningBal };
            });
            ...
            document.getElementById('historyBody').innerHTML = historyTemp.reverse().map(h => \`
                <tr class="border-b border-zinc-800/30 \${h.isRecovery?'recovery-row':''}">
                    ...
                    <td><b>\${h.symbol}</b></td><td>\${h.dcaCount}</td><td>\${h.totalMargin.toFixed(1)}</td>
                    <td>\${fPrice(h.snapPrice)}/\${fPrice(h.finalPrice)}</td><td>\${h.snapVol.c1}/\${h.snapVol.c5}</td>
