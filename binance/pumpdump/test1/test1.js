    ...
    ws.on('message', (data) => {
        ...
        tickers.forEach(t => {
            ...
            if (pending) {
                ...
                // Cập nhật MaxDD chi tiết (ROI, PnL, Time)
                if (!pending.maxNegativeRoi || currentRoi < pending.maxNegativeRoi) {
                    pending.maxNegativeRoi = currentRoi;
                    pending.maxNegativeTime = now;
                    // Tạm tính PnL âm nhất để lưu vào lịch sử
                    const savedState = JSON.parse(fs.readFileSync('./luffy_state_cache.json', 'utf8') || '{}');
                    const mVal = savedState.marginVal || "10";
                    const mBase = mVal.includes('%') ? (1000 * parseFloat(mVal) / 100) : parseFloat(mVal); // 1000 là số giả định, sẽ tính chuẩn lúc render
                    pending.maxNegativePnl = (mBase * (pending.dcaCount + 1)) * currentRoi / 100;
                }
...
app.get('/gui', (req, res) => {
    ...
    <div class="px-4 mt-5"><div class="bg-card rounded-xl p-4 shadow-lg">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[10px] text-left"><thead class="text-gray-custom uppercase border-b border-zinc-800"><tr><th>STT</th><th>Time</th><th>Pair</th><th>DCA</th><th>Margin</th><th>Entry/Live</th><th>Avg Price</th><th class="text-right">PnL (ROI%)</th></tr></thead><tbody id="pendingBody"></tbody></table></div>
    </div></div>

    <div class="px-4 mt-5"><div class="bg-card rounded-xl p-4 shadow-lg">
         <div class="text-[11px] font-bold text-gray-custom mb-3 uppercase tracking-wider italic">Nhật ký giao dịch</div>
        <div class="overflow-x-auto"><table class="w-full text-[9px] text-left"><thead class="text-gray-custom border-b border-zinc-800 uppercase"><tr><th>STT</th><th>Time In-Out</th><th>Pair</th><th>DCA</th><th>Margin</th><th>Entry/Out</th><th>Avg Price</th><th class="text-center">MaxDD (Time/$)</th><th>PnL Net</th><th class="text-right">Balance</th></tr></thead><tbody id="historyBody"></tbody></table></div>
    </div></div>
    ...
    async function update() {
        try {
            ...
            let histHTML = [...d.history].reverse().map((h, idx) => {
                ...
                // Hiển thị MaxDD: ROI | Tiền | Giờ
                let maxDDStr = \`<span class="down font-bold">\${h.maxNegativeRoi.toFixed(1)}%</span><br><span class="text-[7px] text-gray-500">\${h.maxNegativeTime ? new Date(h.maxNegativeTime).toLocaleTimeString([],{hour12:false}) : '--'}<br>(\${(h.maxNegativePnl || 0).toFixed(1)}$)</span>\`;

                return \`<tr class="border-b border-zinc-800/30"><td>\${d.history.length - idx}</td><td class="py-2 text-[7px]">\${new Date(h.startTime).toLocaleTimeString([],{hour12:false})}<br>\${new Date(h.endTime).toLocaleTimeString([],{hour12:false})}</td><td><b class="text-white">\${h.symbol}</b> <br> <span class="\${h.type==='LONG'?'up':'down'} font-bold">\${h.type}</span></td><td>\${h.dcaCount}</td><td>\${totalMargin.toFixed(1)}</td><td>\${fPrice(h.snapPrice)}<br>\${fPrice(h.finalPrice)}</td><td>\${fPrice(h.avgPrice)}</td><td class="text-center">\${maxDDStr}</td><td class="\${pnl>=0?'up':'down'} font-bold">\${pnl.toFixed(2)}</td><td class="text-right text-white">\${runningBal.toFixed(1)}</td></tr>\`;
            }).reverse().join('');
            ...
            let pendingHTML = d.pending.map((h, idx) => {
                ...
                return \`<tr class="bg-white/5 border-b border-zinc-800"><td>\${idx+1}</td><td class="text-[8px]">\${new Date(h.startTime).toLocaleTimeString([],{hour12:false})}</td><td class="text-white font-bold">\${h.symbol} <span class="text-[8px] px-1 \${h.type==='LONG'?'bg-green-600':'bg-red-600'} rounded">\${h.type}</span></td><td class="text-yellow-500 font-bold">\${h.dcaCount}</td><td>\${totalM.toFixed(1)}</td><td>\${fPrice(h.snapPrice)}<br><b class="text-green-400">\${fPrice(lp)}</b></td><td class="text-yellow-500 font-bold">\${fPrice(h.avgPrice)}</td><td class="text-right font-bold \${pnl>=0?'up':'down'}">\${pnl.toFixed(2)}<br>\${roi.toFixed(1)}%</td></tr>\`;
            }).join('');
    ...
    myChart = new Chart(ctx, { type: 'line', data: { labels: [], datasets: [{ data: [], borderColor: '#fcd535', borderWidth: 2, pointRadius: 0, fill: false }] }, options: { maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { display: false }, y: { grid: { color: '#30363d' } } } } });
