...
app.get('/gui', (req, res) => {
    ...
    <div id="statsModal" class="modal">
        <div class="bg-card p-4 rounded-lg w-full max-w-3xl border border-zinc-700 max-h-[90vh] flex flex-col">
            <div class="flex justify-between mb-4">
                <h2 id="statsModalTitle" class="text-yellow-500 font-bold uppercase italic"></h2>
                <button onclick="closeModal('statsModal')" class="text-2xl text-gray-400">&times;</button>
            </div>
            <div class="overflow-y-auto scroll-custom flex-1">
                <table class="w-full text-[10px] text-left">
                    <thead class="text-gray-400 border-b border-zinc-800"><tr><th>STT</th><th>Thời gian</th><th>Entry/Out</th><th>DCA</th><th>ROI</th><th>PnL Net</th></tr></thead>
    ...
    <div id="detailModal" class="modal">
        <div class="bg-card p-5 rounded-lg w-11/12 max-w-xl border border-zinc-700 relative">
            <button onclick="closeModal('detailModal')" class="absolute top-2 right-4 text-2xl text-gray-400">&times;</button>
            <h2 id="modalTitle" class="text-yellow-500 font-bold mb-4 uppercase text-center"></h2>
            <div id="modalSummary" class="grid grid-cols-3 gap-2 mb-4 text-center"></div>
            <table class="w-full text-[10px] text-left">
                <thead class="text-gray-400 border-b border-zinc-800"><tr><th>Lần</th><th>Thời gian</th><th>Giá DCA</th><th>Giá TB</th></tr></thead>
                <tbody id="modalBody"></tbody>
            </table>
        </div>
    </div>

    <div class="p-4 bg-[#0b0e11] border-b border-zinc-800 sticky top-0 z-40">
        <div id="setup" class="grid grid-cols-2 gap-2 mb-4 bg-card p-3 rounded-lg">
            <input id="balanceInp" type="number" placeholder="Vốn đầu" class="bg-[#0b0e11] p-2 rounded text-yellow-500 font-bold">
        ...
        <div class="flex justify-between items-end">
            <div><div class="text-gray-400 text-[10px] uppercase font-bold">Balance Realtime</div><span id="displayBal" class="text-4xl font-bold tracking-tighter">0.00</span></div>
            <div class="text-right"><div class="text-gray-400 text-[10px] uppercase font-bold">PnL Tạm tính</div><div id="unPnl" class="text-xl font-bold">0.00</div></div>
        </div>
    </div>
    ...
    <div class="p-4 space-y-6">
        <div>
            <div class="text-[10px] font-bold text-green-500 uppercase mb-2">Vị thế đang chạy</div>
            <div class="bg-card rounded-lg overflow-x-auto"><table class="w-full text-[10px] text-left">
                <thead class="text-gray-400 border-b border-zinc-800 uppercase"><tr><th class="p-2">Pair</th><th>DCA</th><th>Entry/Live</th><th>ROI%</th><th class="text-right p-2">PnL ($)</th></tr></thead>
                <tbody id="pendingBody"></tbody>
            </table></div>
        </div>

        <div>
            <div class="text-[10px] font-bold text-gray-400 uppercase mb-2">Nhật ký chốt lệnh</div>
            <div class="bg-card rounded-lg overflow-x-auto"><table class="w-full text-[9px] text-left">
                <thead class="text-gray-400 border-b border-zinc-800 uppercase"><tr><th class="p-2">Time</th><th>Pair</th><th>DCA</th><th>Entry/Out</th><th>ROI%</th><th class="text-right p-2">PnL Net</th></tr></thead>
        ...
        <div>
            ...
            <div class="bg-card rounded-lg overflow-x-auto"><table class="w-full text-[10px] text-left">
                <thead class="text-gray-400 border-b border-zinc-800 uppercase"><tr><th class="p-2">Coin</th><th>Lệnh</th><th>DCA</th><th>Chốt Lời</th><th>Tạm Tính</th><th class="text-right p-2">Tổng PnL</th></tr></thead>
    ...
    function fDate(t) {
        const d = new Date(t);
        return d.toLocaleTimeString('vi-VN', {hour12:false}) + ' - ' + d.getDate() + '/' + (d.getMonth()+1);
    }
    ...
    // Hiện toàn bộ lịch sử của 1 Coin
    function showStatsModal(sym) {
        const history = lastRawData.history.filter(h => h.symbol === sym);
        document.getElementById('statsModalTitle').innerText = 'LỊCH SỬ GIAO DỊCH: ' + sym;
        document.getElementById('statsModalBody').innerHTML = history.map((h, i) => {
            ...
            return \`<tr class="border-b border-zinc-800/30 cursor-pointer hover:bg-white/5" onclick="showDetail('\${h.symbol}', \${h.startTime})">
                ...
                <td>\${fDate(h.startTime)}</td>
                <td>\${fPrice(h.snapPrice)} → \${fPrice(h.finalPrice)}</td>
                <td class="text-yellow-500 font-bold">\${h.dcaCount}</td>
                <td class="font-bold \${h.pnlPercent>=0?'up':'down'}">\${h.pnlPercent.toFixed(2)}%</td>
                <td class="font-bold \${netPnl>=0?'up':'down'}">\${netPnl.toFixed(2)}</td>
            </tr>\`;
        }).join('');
    ...
    function syncConfig() {
        fetch(\`/api/config?tp=\${document.getElementById('tpInp').value}&sl=\${document.getElementById('slInp').value}\`);
    }
    function start() {
        localStorage.setItem('luffy_state', JSON.stringify({ running: true, initialBal: parseFloat(document.getElementById('balanceInp').value), marginVal: document.getElementById('marginInp').value, tp: document.getElementById('tpInp').value, sl: document.getElementById('slInp').value }));
        location.reload();
    }
    ...
    function initChart() {
        ...
        myChart = new Chart(ctx, {
            ...
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { display: false }, y: { grid: { color: '#1e2329' }, ticks: { font: { size: 9 } } } } }
        });
    }
    ...
    async function update() {
        try {
            ...
            let mVal = document.getElementById('marginInp').value;
            let runningBal = initialBal, totalUnPnl = 0, coinStats = {};

            // Render Nhật ký
            document.getElementById('historyBody').innerHTML = [...d.history].reverse().map(h => {
                let marginBase = mVal.includes('%') ? (runningBal * parseFloat(mVal) / 100) : parseFloat(mVal);
                let totalMargin = marginBase * (h.dcaCount + 1);
                let netPnl = (totalMargin * (h.maxLev || 20) * (h.pnlPercent/100)) - (totalMargin * (h.maxLev || 20) * 0.001);
                runningBal += netPnl;
                
                if(!coinStats[h.symbol]) coinStats[h.symbol] = { count:0, dca:0, winPnl:0, livePnl:0 };
                coinStats[h.symbol].count++; coinStats[h.symbol].dca += h.dcaCount;
                if(netPnl >= 0) coinStats[h.symbol].winPnl += netPnl; else coinStats[h.symbol].winPnl += netPnl;

                return \`<tr class="border-b border-zinc-800/30 text-zinc-400">
                    <td class="p-2">\${fDate(h.endTime)}</td>
                    <td class="font-bold text-white cursor-pointer" onclick="showDetail('\${h.symbol}', \${h.startTime})">\${h.symbol}</td>
                    <td class="text-yellow-500">\${h.dcaCount}</td>
                    <td>\${fPrice(h.snapPrice)}/\${fPrice(h.finalPrice)}</td>
                    <td class="\${h.pnlPercent>=0?'up':'down'}">\${h.pnlPercent.toFixed(2)}%</td>
                    <td class="text-right p-2 font-bold \${netPnl>=0?'up':'down'}">\${netPnl.toFixed(2)}</td>
                </tr>\`;
            }).join('');
            ...
            document.getElementById('pendingBody').innerHTML = d.pending.map(h => {
                ...
                let pnl = totalMargin * roi / 100; totalUnPnl += pnl;

                if(!coinStats[h.symbol]) coinStats[h.symbol] = { count:0, dca:0, winPnl:0, livePnl:0 };
                coinStats[h.symbol].livePnl += pnl;

                return \`<tr class="bg-white/5 border-b border-zinc-800">
                    <td class="p-2 font-bold text-yellow-500 cursor-pointer" onclick="showDetail('\${h.symbol}', \${h.startTime})">\${h.symbol} <span class="text-[8px] text-gray-500">\${h.type}</span></td>
                    <td class="font-bold">\${h.dcaCount}</td>
                    <td>\${fPrice(h.avgPrice)} → \${fPrice(lp)}</td>
                    <td class="font-bold \${roi>=0?'up':'down'}">\${roi.toFixed(2)}%</td>
                    <td class="text-right p-2 font-bold \${pnl>=0?'up':'down'}">\${pnl.toFixed(2)}</td>
                </tr>\`;
            }).join('');
            ...
            document.getElementById('statsBody').innerHTML = Object.entries(coinStats).map(([sym, s]) => \`
                <tr class="border-b border-zinc-800/50">
                    <td class="p-2 font-bold text-white cursor-pointer underline" onclick="showStatsModal('\${sym}')">\${sym}</td>
                    <td>\${s.count}</td><td>\${s.dca}</td>
                    <td class="up font-bold">\${s.winPnl.toFixed(2)}</td>
                    <td class="\${s.livePnl>=0?'up':'down'}">\${s.livePnl.toFixed(2)}</td>
                    <td class="text-right p-2 font-bold \${(s.winPnl+s.livePnl)>=0?'up':'down'}">\${(s.winPnl+s.livePnl).toFixed(2)}</td>
                </tr>\`).join('');

            let now = Date.now();
            let currentTotal = runningBal + totalUnPnl;
            if (now - lastHistorySave > 600000) { 
                balanceHistory.push({ t: now, v: currentTotal });
                if(balanceHistory.length > 500) balanceHistory.shift();
                localStorage.setItem('luffy_hist_v2', JSON.stringify(balanceHistory));
                lastHistorySave = now;
            }

            if(myChart) {
                myChart.data.labels = [...balanceHistory.map(i => fDate(i.t)), "REALTIME"];
                myChart.data.datasets[0].data = [...balanceHistory.map(i => i.v), currentTotal];
                myChart.update('none');
            }

            if(running) {
                document.getElementById('displayBal').innerText = currentTotal.toFixed(2);
                document.getElementById('displayBal').className = 'text-4xl font-bold tracking-tighter ' + (currentTotal >= initialBal ? 'up' : 'down');
                document.getElementById('unPnl').innerText = (totalUnPnl >= 0 ? '+' : '') + totalUnPnl.toFixed(2);
                document.getElementById('unPnl').className = 'text-xl font-bold ' + (totalUnPnl >= 0 ? 'up' : 'down');
            }
        } catch(e) {}
    }
