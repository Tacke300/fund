    ...
    ws.on('message', (data) => {
        ...
        tickers.forEach(t => {
            ...
            if (Math.max(Math.abs(c1), Math.abs(c5), Math.abs(c15)) >= MIN_VOLATILITY_TO_SAVE && !pending && !isCooldown) {
                historyMap.set(`${s}_${now}`, { symbol: s, startTime: now, snapPrice: p, type: (c1+c5+c15 >= 0) ? 'UP' : 'DOWN', status: 'PENDING', maxLev: symbolMaxLeverage[s] || 20, snapVol: { c1, c5, c15 }, tpTarget: TP_PERCENT, slTarget: SL_PERCENT });
            }
        });
    });
...
app.get('/gui', (req, res) => {
    ...
    <title>Binance Luffy Pro - ORIGINAL LAYOUT</title><script src="https://cdn.tailwindcss.com"></script>
    <style>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-[9px] text-left">
                    <thead class="text-gray-custom uppercase border-b border-zinc-800">
                        <tr><th class="pb-2">Time</th><th class="pb-2">Coin</th><th class="pb-2">Margin</th><th class="pb-2 text-center">Lev/Target</th><th class="pb-2">Entry/Live</th><th class="pb-2 text-right">PnL (ROI%)</th></tr>
                    </thead>
    ...
    <div class="px-4 mt-4 pb-32">
        <div class="bg-card rounded-lg p-3">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-[9px] text-left">
                    <thead class="text-gray-custom uppercase border-b border-zinc-800">
                        <tr><th class="pb-2">Time</th><th class="pb-2">Coin</th><th class="pb-2">Margin</th><th class="pb-2 text-center">Lev/Target</th><th class="pb-2">Entry/Exit</th><th class="pb-2 text-white">PnL Net</th><th class="pb-2 text-right">Balance</th></tr>
                    </thead>
    ...
    async function update() {
        try {
            ...
            let runningBal = initialBal, stats = { winCount: 0, winSum: 0, loseCount: 0, loseSum: 0 };
            let historyHTML = [...(d.history || [])].reverse().map(h => {
                ...
                return \`<tr><td class="py-2 text-zinc-500">\${new Date(h.startTime).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td>
                    <td><b class="text-white">\${h.symbol}</b><br><span class="\${h.type==='UP'?'up':'down'}">\${h.type}</span></td>
                    <td>\${margin.toFixed(1)}</td><td class="text-center">\${h.maxLev}x<br><span class="text-[#fcd535]">\${fPrice(tp)}/\${fPrice(sl)}</span></td>
                    <td>\${fPrice(h.snapPrice)}<br>\${fPrice(h.finalPrice)}</td><td class="font-bold \${netPnl>=0?'up':'down'}">\${netPnl.toFixed(2)}</td>
                    <td class="text-right">\${runningBal.toFixed(1)}</td></tr>\`;
            }).reverse().join('');
            ...
            document.getElementById('pendingBody').innerHTML = (d.pending || []).map(h => {
                ...
                let tp = h.type==='UP' ? h.snapPrice*(1+h.tpTarget/100) : h.snapPrice*(1-h.tpTarget/100);
                let sl = h.type==='UP' ? h.snapPrice*(1-h.slTarget/100) : h.snapPrice*(1+h.slTarget/100);
                return \`<tr class="bg-green-500/5"><td class="py-2 text-zinc-500">\${new Date(h.startTime).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit',second:'2-digit'})}</td>
                    <td><b class="text-white">\${h.symbol}</b><br><span class="\${h.type==='UP'?'up':'down'}">\${h.type}</span></td>
                    <td>\${margin.toFixed(1)}</td><td class="text-center">\${h.maxLev}x<br><span class="text-[#fcd535]">\${fPrice(tp)}/\${fPrice(sl)}</span></td>
            ...
            document.getElementById('winCount').innerText = stats.winCount; document.getElementById('winSum').innerText = '+' + stats.winSum.toFixed(2);
            document.getElementById('loseCount').innerText = stats.loseCount; document.getElementById('loseSum').innerText = stats.loseSum.toFixed(2);
            if(running) {
