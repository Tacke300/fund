...
// --- LOGIC KIỂM TRA NẾN CŨ (BACK-CHECK) ---
...
app.get('/gui', (req, res) => {
    ...
    <title>Binance Futures Pro</title><script src="https://cdn.tailwindcss.com"></script><script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    ...
    <style>
        ...
        .action-btn { background: #2b3139; color: #eaecef; padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 600; flex: 1; text-align: center; }
    ...
    <div class="p-4 bg-[#0b0e11] border-b border-zinc-800">
        <div class="flex items-center gap-1 text-zinc-400 text-sm mb-1">
            <span>Số dư margin</span> <i class="far fa-eye text-xs"></i>
        </div>
        <div class="flex items-center gap-2 mb-1">
            <span id="displayBal" class="text-3xl font-bold tracking-tight">0.00</span>
            <span class="text-lg font-medium">USDT</span>
            <i class="fas fa-caret-down text-zinc-500"></i>
        </div>
        <div class="text-zinc-500 text-sm mb-4 italic">≈đ0.00</div>
        
        <div class="flex justify-between items-center mb-6 text-sm">
            <span class="text-zinc-400">Lãi lỗ đã ghi nhận hôm nay</span>
            <span class="text-[#0ecb81] font-medium">$0.00(0.00%) <i class="fas fa-chevron-right text-[10px]"></i></span>
        </div>

        <div class="grid grid-cols-2 gap-4 mb-6">
            <div>
                <div class="text-zinc-500 text-xs mb-1">Số dư ví (USDT)</div>
                <div id="walletBal" class="font-bold">0.00</div>
                <div class="text-zinc-500 text-[10px] italic">≈đ0.00</div>
            </div>
            <div>
                <div class="text-zinc-500 text-xs mb-1">PNL chưa được ghi nhận (USDT)</div>
                <div id="unPnl" class="font-bold">0.00</div>
                <div class="text-zinc-500 text-[10px] italic">đ0.000000</div>
            </div>
        </div>

        <div class="flex gap-2">
            <div class="action-btn yellow-btn" onclick="start()">Giao dịch</div>
            <div class="action-btn">Hoán đổi</div>
            <div class="action-btn">Chuyển</div>
        </div>
    </div>
    ...
    <div class="px-4">
        <div class="flex gap-4 mb-4 border-b border-zinc-800 pb-2 text-sm font-bold">
        ...
        <div id="pendingContainer" class="space-y-6">
            <div id="emptyState" class="text-center py-10">
                <i class="fas fa-file-invoice text-4xl text-zinc-700 mb-2"></i>
                <div class="text-zinc-500 text-sm">Bạn không có vị thế nào</div>
            </div>
        </div>
    </div>

    <div id="setup" class="fixed bottom-0 left-0 right-0 bg-[#1e2329] p-4 border-t border-zinc-700 z-[100]">
        <div class="flex gap-4 mb-4">
            <div class="flex-1">
                <label class="text-[10px] text-zinc-500 font-bold">VỐN GỐC</label>
                <input id="balanceInp" type="number" value="1000" class="w-full bg-transparent border-b border-zinc-600 outline-none py-1 font-bold">
            </div>
            <div class="flex-1">
                <label class="text-[10px] text-zinc-500 font-bold">MARGIN %</label>
                <input id="marginInp" type="text" value="10%" class="w-full bg-transparent border-b border-zinc-600 outline-none py-1 font-bold">
            </div>
        </div>
        <button onclick="start()" class="w-full bg-[#fcd535] text-black py-3 rounded-lg font-bold">KÍCH HOẠT BOT</button>
    </div>
    ...
    if(localStorage.getItem('bot_luffy_v3')) {
        ...
        if(running) document.getElementById('setup').classList.add('hidden');
    }
    ...
    function start() { running = true; initialBal = parseFloat(document.getElementById('balanceInp').value); currentBal = initialBal; historyLog = [{t: Date.now(), b: initialBal}]; document.getElementById('setup').classList.add('hidden'); save(); }
    ...
    async function update() {
        try {
                    ...
                    <div class="mb-8">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2">
                                ...
                                <i class="fas fa-chart-bar text-zinc-600 text-xs"></i>
                            </div>
                        ...
                        <div class="grid grid-cols-2 mb-4">
                            <div>
                                <div class="text-zinc-500 text-xs dot-border inline-block mb-1">PNL (USDT)</div>
                                <div class="text-2xl font-bold \${pnl>=0?'up':'down'}">\${pnl.toFixed(2)}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-zinc-500 text-xs inline-block mb-1">ROI</div>
                                <div class="text-2xl font-bold \${pnl>=0?'up':'down'}">\${roi >=0 ? '+' : ''}\${roi.toFixed(2)}%</div>
                            </div>
                        </div>

                        <div class="grid grid-cols-3 text-xs mb-3">
                            <div>
                                <div class="text-zinc-500 mb-1">Kích thước (USDT)</div>
                                <div class="font-medium">\${(margin*(h.maxLev||0)).toFixed(2)}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-zinc-500 dot-border inline-block mb-1">Margin (USDT)</div>
                                <div class="font-medium">\${margin.toFixed(2)}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-zinc-500 dot-border inline-block mb-1">Tỉ lệ ký quỹ</div>
                                <div class="text-[#0ecb81] font-medium">6.51%</div>
                            </div>
                        </div>

                        <div class="grid grid-cols-3 text-xs mb-5">
                            <div>
                                <div class="text-zinc-500 dot-border inline-block mb-1">Giá vào lệnh (USDT)</div>
                                <div class="font-medium">\${h.snapPrice.toFixed(6)}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-zinc-500 dot-border inline-block mb-1">Giá đánh dấu (USDT)</div>
                                <div class="font-medium text-zinc-300">\${livePrice.toFixed(6)}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-zinc-500 dot-border inline-block mb-1">Giá thanh lý (USDT)</div>
                                <div class="font-medium text-orange-200">--</div>
                            </div>
                        </div>

                        <div class="grid grid-cols-3 gap-2">
                            <div class="binance-btn">Đòn bẩy</div>
                            <div class="binance-btn">TP/SL</div>
                            <div class="binance-btn">Đóng</div>
                        </div>
                    </div>\`;
                }).join('');
            } else {
            ...
            if(running) {
                ...
                document.getElementById('displayBal').innerText = currentBal.toFixed(8);
                document.getElementById('walletBal').innerText = (initialBal + totalClosedP).toFixed(8);
