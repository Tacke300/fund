...
app.get('/gui', (req, res) => {
    ...
    <div class="p-4 bg-[#0b0e11] sticky top-0 z-50 border-b border-zinc-800">
        ...
        <div class="flex justify-between items-end mb-3">
            <div><div class="text-gray-custom text-[11px] uppercase font-bold tracking-widest mb-1">Tổng tài sản ước tính</div><span id="displayBal" class="text-4xl font-bold text-white tracking-tighter">0.00</span><span class="text-sm text-gray-custom ml-1">USDT</span></div>
        ...
        <div class="grid grid-cols-2 gap-4 text-[11px] border-t border-zinc-800 pt-3 mb-3 uppercase font-bold">
            <div><span class="text-gray-custom">Ký quỹ: </span><span id="walletBal" class="text-white">0.00</span></div>
    ...
    <div class="px-4 mt-5">
        <div class="bg-card rounded-xl p-4">
            <div class="text-[11px] font-bold text-yellow-500 mb-2 uppercase tracking-widest italic">Biến động tổng tài sản (Balance Chart)</div>
    ...
    <div class="px-4 mt-5"><div class="bg-card rounded-xl p-4 shadow-lg">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[9px] text-left"><thead class="text-gray-custom border-b border-zinc-800 uppercase"><tr><th>STT</th><th>Time In-Out</th><th>Pair/Vol</th><th>DCA</th><th>Margin</th><th class="text-center">Target</th><th>Entry/Out</th><th>Avg Price</th><th class="text-center">Max Drawdown</th><th>PnL Net</th><th class="text-right">Balance</th></tr></thead><tbody id="historyBody"></tbody></table></div>
    </div></div>
    ...
    <div class="px-4 mt-5 pb-32"><div class="bg-card rounded-xl p-4 shadow-lg border border-yellow-500/20">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[10px] text-left"><thead class="text-gray-custom border-b border-zinc-800 uppercase"><tr><th>STT</th><th>Tên Coin</th><th>Lev</th><th>Lệnh</th><th>DCA</th><th>PnL Lãi</th><th>PnL Lỗ</th><th class="text-right">Tổng PnL</th></tr></thead><tbody id="statsBody"></tbody></table></div>
    </div></div>
    ...
    function initChart() {
        ...
        myChart = new Chart(ctx, {
            type: 'line',
            data: { labels: [], datasets: [{ label: 'Balance', data: [], borderColor: '#fcd535', borderWidth: 2, fill: true, backgroundColor: 'rgba(252, 213, 53, 0.05)', tension: 0.3, pointRadius: 0, pointHoverRadius: 5 }] },
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false }, tooltip: { 
                    mode: 'index', intersect: false,
                    callbacks: { label: function(ctx) { 
                        let val = ctx.raw; let pnl = val - initialBal;
                        return [\`Balance: \${val.toFixed(2)} USDT\`, \`Total PnL: \${pnl.toFixed(2)} USDT\`]; 
                    } }
                } },
                scales: { 
                    x: { display: false }, 
    ...
    async function update() {
        try {
            ...
            let runningBal = initialBal, winSum = 0, loseSum = 0, winCount = 0, loseCount = 0, coinStats = {};
            ...
            document.getElementById('statsBody').innerHTML = Object.entries(coinStats).map(([sym, s], i) => \`<tr class="border-b border-zinc-800/50"><td>\${i+1}</td><td class="text-white font-bold">\${sym}</td><td>\${s.lev}x</td><td>\${s.count}</td><td class="text-yellow-500">\${s.dcas}</td><td class="up">\${s.pnlW.toFixed(2)}</td><td class="down">\${s.pnlL.toFixed(2)}</td><td class="text-right font-bold \${(s.pnlW+s.pnlL)>=0?'up':'down'}">\${(s.pnlW+s.pnlL).toFixed(2)}</td></tr>\`).join('');
