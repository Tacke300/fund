...
app.get('/gui', (req, res) => {
    ...
    <div class="p-4 bg-main border-b border-zinc-800">
        ...
        <div id="active" class="hidden flex justify-between items-center mb-4">
             <div class="flex items-center gap-2"><img src="https://bin.bnbstatic.com/static/images/common/favicon.ico" class="w-5"><h1 class="font-bold italic text-sm">BINANCE <span class="text-[#fcd535]">PRO</span></h1></div>
        ...
        <div class="grid grid-cols-3 gap-2 mb-6 text-center">
            <div class="bg-card p-2 rounded border border-zinc-800"><div class="text-gray-custom text-[10px]">TODAY (7H)</div><div id="stat24" class="font-bold text-[11px]">---</div></div>
            <div class="bg-card p-2 rounded border border-zinc-800"><div class="text-gray-custom text-[10px]">7 DAYS</div><div id="stat7" class="font-bold text-[11px]">---</div></div>
            <div class="bg-card p-2 rounded border border-zinc-800"><div class="text-gray-custom text-[10px]">30 DAYS</div><div id="stat30" class="font-bold text-[11px]">---</div></div>
        </div>
    ...
    // --- CẤU HÌNH BIỂU ĐỒ TỪ CODE GỐC ---
    const chart = new Chart(document.getElementById('mainChart').getContext('2d'), {
        type: 'line', 
        data: { 
            labels: historyLog.map((_,i)=>i), 
            datasets: [{ 
                data: historyLog.map(pt => pt.b), 
                borderColor: '#fcd535', 
                tension: 0.3, 
                pointRadius: 0, 
                fill: true, 
    ...
    async function update() {
        try {
            ...
            document.getElementById('pendingContainer').innerHTML = d.pending.map(h => {
                ...
                return `<div>
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="bg-[#2b3139] px-1 rounded text-gray-custom text-[10px]">Cross ${h.maxLev}X</span>
                    </div>
                    ...
                    <div class="grid grid-cols-3 text-[11px] mb-4 text-gray-custom border-b border-zinc-800 pb-4">
                        <div><div>Kích thước</div><div class="text-white font-medium">${(margin*h.maxLev).toFixed(2)}</div></div>
                        <div class="text-center"><div>Giá vào lệnh</div><div class="text-white font-medium">${h.snapPrice.toFixed(4)}</div></div>
                        <div class="text-right"><div>Giá đánh dấu</div><div class="text-white font-medium">${livePrice.toFixed(4)}</div></div>
                    </div>
            ...
            document.getElementById('historyBody').innerHTML = d.history.map(h => {
                ...
                return `<tr class="border-b border-zinc-800"><td class="py-2 text-gray-custom">${new Date(h.startTime).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</td><td class="font-bold text-zinc-300">${h.symbol} ${h.maxLev}x</td><td class="text-right font-bold ${pnl>=0?'up':'down'}">${pnl.toFixed(1)}$</td><td class="text-right font-black ${h.status==='WIN'?'up':'down'}">${h.status}</td></tr>`;
            }).join('');
