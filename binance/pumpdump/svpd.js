...
const BALANCE_LOG_FILE = './balance_history.json'; // File lưu biểu đồ
...
let lastTradeClosed = {}; 
let balanceLogs = []; // Lưu mốc 10 phút
...
setInterval(() => {
    ...
    // Tính toán số dư thực tế dựa trên lịch sử (giả định bắt đầu từ vốn config)
    let configData = { initialBal: 1000, marginVal: "10%" }; // Mặc định nếu chưa có
    // Lưu ý: Số dư này sẽ được tính toán chính xác hơn ở Client, 
    // ở đây ta lưu mốc thời gian và tổng PnL đã chốt để tái cấu trúc biểu đồ.
    const currentPnl = hist.reduce((sum, h) => {
        let marginBase = 10; // Giả định margin cơ bản
    ...
    // Giới hạn lưu 1000 điểm (khoảng 7 ngày nếu mỗi 10p/điểm)
...
app.get('/api/data', (req, res) => {
    ...
    res.json({ 
        ...
        balanceLogs: balanceLogs // Gửi dữ liệu biểu đồ 10p về
    });
});
...
app.get('/gui', (req, res) => {
    ...
    <div class="px-4 mt-5">
        <div class="bg-card rounded-xl p-4">
            <div class="text-[11px] font-bold text-yellow-500 uppercase tracking-widest italic mb-2">Biến động tài sản (Log 10m/điểm)</div>
    ...
    function initChart() {
        ...
        myChart = new Chart(ctx, {
            ...
            data: { labels: [], datasets: [{ 
                ...
                segment: {
                    borderColor: ctx => ctx.p1.raw < initialBal ? '#f6465d' : '#0ecb81',
                    backgroundColor: ctx => ctx.p1.raw < initialBal ? 'rgba(246, 70, 93, 0.05)' : 'rgba(14, 203, 129, 0.05)'
                }
            }] },
    ...
    async function update() {
        try {
            ...
            // Xử lý Market Volatility
            document.getElementById('marketBody').innerHTML = (d.live || []).slice(0, 15).map(m => \`
                <tr class="border-b border-zinc-800/30 text-[11px]">
                    <td class="font-bold text-white py-2">\${m.symbol}</td><td class="text-yellow-500">\${fPrice(m.currentPrice)}</td>
            ...
            // Cập nhật BIỂU ĐỒ từ LOG 10 phút
            if(myChart && d.balanceLogs) {
                let chartLabels = d.balanceLogs.map(l => new Date(l.t).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}));
                let chartValues = d.balanceLogs.map(l => initialBal + l.pnl);
                
                // Điểm Live cuối cùng
                chartLabels.push("LIVE");
                chartValues.push(runningBal + unPnl);
