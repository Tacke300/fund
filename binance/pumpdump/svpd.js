...
app.get('/gui', (req, res) => {
    ...
    <div class="p-4 bg-[#0b0e11]">
        <div class="grid grid-cols-3 gap-2 mb-6 text-center">
            <div class="bg-card p-2 rounded"><div class="text-gray-custom text-[10px] uppercase">Hôm nay</div><div id="stat24" class="font-bold text-xs text-white">---</div></div>
            <div class="bg-card p-2 rounded"><div class="text-gray-custom text-[10px] uppercase">7 Ngày</div><div id="stat7" class="font-bold text-xs text-white">---</div></div>
            <div class="bg-card p-2 rounded"><div class="text-gray-custom text-[10px] uppercase">30 Ngày</div><div id="stat30" class="font-bold text-xs text-white">---</div></div>
        </div>
        ...
        <div class="text-gray-custom text-xs mb-1">Số dư ký quỹ <i class="far fa-eye text-[10px]"></i></div>
    ...
    <div class="px-4 mb-4 mt-2">
        <div class="bg-card rounded p-3">
            <div class="text-[10px] font-bold text-gray-custom mb-2 uppercase border-b border-zinc-800 pb-1 italic">Tín hiệu biến động (1m | 5m | 15m)</div>
    ...
    <div class="px-2 pb-24">
        <div class="bg-card rounded p-3 mx-2">
            <div class="text-[10px] font-bold text-gray-custom mb-3 uppercase border-b border-zinc-800 pb-1">Lịch sử chốt lệnh (15p Cooldown)</div>
            <div class="overflow-x-auto"><table class="w-full text-[10px] text-left min-w-[600px]">
                <thead class="text-gray-custom uppercase"><tr><th class="pb-2">Time Close</th><th class="pb-2">Coin</th><th class="pb-2">Entry</th><th class="pb-2">TP/SL</th><th class="pb-2">Max Lev/Margin</th><th class="pb-2 text-right">PNL</th></tr></thead>
    ...
    // HÀM HIỂN THỊ GIÁ THÔNG MINH (SMART DECIMAL)
    function fP(price) {
        if (!price) return "0.00";
        let p = parseFloat(price);
        if (p < 0.0001) return p.toFixed(8); // Ví dụ: 0.00008888
        if (p < 1) return p.toFixed(4);      // Ví dụ: 0.4566
    ...
    async function update() {
        try {
            ...
            document.getElementById('pendingContainer').innerHTML = d.pending.map(function(h){
                ...
                var dots = '';
                if(marginPct > 30) dots = '<span class="down ml-1 font-black">!!!!</span>';
                else if(marginPct < 10) dots = '<span class="up ml-1 font-black">!!!!</span>';
                else if(marginPct < 20) dots = '<span class="up ml-1 font-black">!!</span>';
                ...
                return '<div class="relative">' +
                    '<div class="flex items-center gap-1 mb-3">' +
                        ...
                        '<span class="text-gray-custom text-[10px] ml-1 bg-[#2b3139] px-1 rounded">Cross ' + (h.maxLev || 20) + 'X</span>' + dots + 
                        '<i class="fas fa-share-alt text-gray-custom ml-auto text-xs"></i>' +
                    '</div>' +
                    '<div class="grid grid-cols-2 mb-4">' +
                        '<div><div class="text-gray-custom text-xs dot-underline mb-1">PnL (USDT)</div><div class="text-xl font-bold ' + (pnl>=0?'up':'down') + '">' + pnl.toFixed(2) + '</div></div>' +
                        '<div class="text-right"><div class="text-gray-custom text-xs dot-underline mb-1">ROI</div><div class="text-xl font-bold ' + (pnl>=0?'up':'down') + '">' + roi.toFixed(2) + '%</div></div>' +
                    '</div>' +
                    '<div class="grid grid-cols-3 text-[11px] mb-3 text-gray-custom">' +
                        ...
                        '<div class="text-right"><div class="dot-underline mb-1">Tỉ lệ ký quỹ</div><div class="up">' + marginPct + '%</div></div>' +
                    '</div>' +
                    '<div class="grid grid-cols-3 text-[11px] mb-4 text-gray-custom">' +
                        '<div><div class="dot-underline mb-1">Giá vào lệnh</div><div class="text-white">' + fP(h.snapPrice) + '</div></div>' +
                        '<div class="text-center"><div class="dot-underline mb-1">Giá đánh dấu</div><div class="text-white">' + fP(livePrice) + '</div></div>' +
                        '<div class="text-right"><div class="dot-underline mb-1">Giá thanh lý</div><div class="text-orange-300">--</div></div>' +
                    '</div>' +
                    '<div class="flex items-center gap-1 text-[11px] mb-5 font-medium">' +
                        '<span class="text-gray-custom">TP/SL vị thế</span>' +
                        '<span class="up">' + fP(tp) + '</span><span class="text-gray-custom">/</span><span class="down">' + fP(sl) + '</span>' +
                    '</div>' +
            ...
            // Lịch sử chốt lệnh giả định PNL để tính balance hiển thị
            document.getElementById('historyBody').innerHTML = d.history.map(function(h){
                var pnl = (h.status === 'WIN' ? 10 : -10);
                totalClosedP += pnl;
                ...
                return '<tr class="border-b border-zinc-800"><td class="py-2 text-gray-custom">' + new Date(h.endTime).toLocaleTimeString() + '</td>' +
                       '<td class="font-bold text-white">' + h.symbol + '</td><td>' + fP(h.snapPrice) + '</td>' +
                       '<td><span class="up">' + fP(h.snapPrice*1.05) + '</span>/<span class="down">' + fP(h.snapPrice*0.95) + '</span></td>' +
                       '<td>' + (h.maxLev||20) + 'x/100</td><td class="text-right font-bold ' + (pnl>=0?'up':'down') + '">' + pnl.toFixed(1) + '</td></tr>';
            }).join('');

            document.getElementById('displayBal').innerText = (initialBal + totalClosedP + totalUnPnl).toLocaleString(undefined, {minimumFractionDigits: 2});
        } catch(e) {}
    }
