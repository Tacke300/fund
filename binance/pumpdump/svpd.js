...
app.get('/gui', (req, res) => {
    ...
    <div id="detailModal" class="modal">
        <div class="bg-card p-6 rounded-lg w-11/12 max-w-xl border border-zinc-700 relative">
            ...
            <div class="overflow-x-auto"><table class="w-full text-[10px] text-left"><thead class="text-gray-custom border-b border-zinc-800"><tr><th>Lần</th><th>Thời gian</th><th>Giá DCA</th><th>Giá TB</th></tr></thead><tbody id="modalBody"></tbody></table></div>
        </div>
    </div>
    ...
    <div class="p-4 bg-[#0b0e11] sticky top-0 z-50 border-b border-zinc-800">
        ...
        <div class="flex justify-between items-end mb-3">
            <div><div class="text-gray-custom text-[11px] uppercase font-bold tracking-widest mb-1">Tài sản Realtime</div><span id="displayBal" class="text-4xl font-bold text-white tracking-tighter">0.00</span><span class="text-sm text-gray-custom ml-1">USDT</span></div>
    ...
    <div class="px-4 mt-5"><div class="bg-card rounded-xl p-4 shadow-lg">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[10px] text-left"><thead class="text-gray-custom uppercase border-b border-zinc-800"><tr><th>Pair</th><th>DCA</th><th>Entry/Live</th><th>ROI%</th><th class="text-right">PnL ($)</th></tr></thead><tbody id="pendingBody"></tbody></table></div>
    </div></div>

    <div class="px-4 mt-5"><div class="bg-card rounded-xl p-4 shadow-lg">
         <div class="text-[11px] font-bold text-gray-custom mb-3 uppercase tracking-wider italic">Nhật ký chốt lệnh</div>
        <div class="overflow-x-auto"><table class="w-full text-[9px] text-left"><thead class="text-gray-custom border-b border-zinc-800 uppercase"><tr><th>Time In/Out</th><th>Pair</th><th>DCA</th><th>Entry/Out</th><th>ROI%</th><th class="text-right">PnL Net</th></tr></thead><tbody id="historyBody"></tbody></table></div>
    </div></div>
    ...
    <div class="px-4 mt-5 pb-32"><div class="bg-card rounded-xl p-4 shadow-lg border border-yellow-500/20">
        ...
        <div class="overflow-x-auto"><table class="w-full text-[10px] text-left"><thead class="text-gray-custom border-b border-zinc-800 uppercase"><tr><th>Tên Coin</th><th>Lệnh</th><th>DCA</th><th>PnL Chốt</th><th>Tạm tính</th><th class="text-right">Tổng PnL</th></tr></thead><tbody id="statsBody"></tbody></table></div>
    </div></div>
    ...
    // Click vào tên coin trong Stats -> Hiện danh sách lệnh đã đánh của coin đó
    ...
    // Click vào dòng lệnh -> Hiện chi tiết DCA (Giống bản cũ của bạn)
    ...
    function syncConfig() {
        fetch(\`/api/config?tp=\${document.getElementById('tpInp').value}&sl=\${document.getElementById('slInp').value}&vol=\${document.getElementById('volInp').value}\`);
    }
    function start() {
    ...
    async function update() {
        try {
            ...
            document.getElementById('historyBody').innerHTML = [...d.history].reverse().map(h => {
                ...
                coinStats[h.symbol].chotPnl += netPnl;

                return \`<tr class="border-b border-zinc-800/30 text-zinc-400">
                    <td class="py-2">\${fDate(h.startTime)}<br>\${fDate(h.endTime)}</td>
                    ...
                    <td class="text-right font-bold \${netPnl>=0?'up':'down'}">\${netPnl.toFixed(2)}</td>
                </tr>\`;
            }).join('');
            ...
            document.getElementById('pendingBody').innerHTML = d.pending.map(h => {
                ...
                return \`<tr class="bg-white/5 border-b border-zinc-800">
                    <td class="p-2 font-bold text-yellow-500 cursor-pointer" onclick="showDetail('\${h.symbol}', \${h.startTime})">\${h.symbol} <small class="text-gray-500">\${h.type}</small></td>
            ...
            document.getElementById('statsBody').innerHTML = Object.entries(coinStats).map(([sym, s]) => \`
                <tr class="border-b border-zinc-800/50">
                    <td class="py-2 font-bold text-white cursor-pointer underline decoration-zinc-600" onclick="showStatsModal('\${sym}')">\${sym}</td>
                    <td>\${s.count}</td><td>\${s.dca}</td>
                    <td class="\${s.chotPnl>=0?'up':'down'}">\${s.chotPnl.toFixed(2)}</td>
                    <td class="\${s.tamPnl>=0?'up':'down'}">\${s.tamPnl.toFixed(2)}</td>
                    <td class="text-right font-bold \${(s.chotPnl+s.tamPnl)>=0?'up':'down'}">\${(s.chotPnl+s.tamPnl).toFixed(2)}</td>
                </tr>\`).join('');

            let now = Date.now();
            let currentTotal = runningBal + totalUnPnl;
            // Lưu history 10 phút / điểm
            if (now - lastHistorySave > 600000) { 
                balanceHistory.push({ t: now, v: currentTotal });
                if(balanceHistory.length > 1000) balanceHistory.shift();
                localStorage.setItem('luffy_v3_hist', JSON.stringify(balanceHistory));
                lastHistorySave = now;
            }

            if(myChart) {
                myChart.data.labels = [...balanceHistory.map(i => i.t), now].map(t => new Date(t).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}));
