...
async function openMarketPosition(symbol, tradeDirection, maxLeverage, entryPriceOverride = null) {
    ...
    try { const details = await getSymbolDetails(symbol); if (!details) throw new Error(`Lỗi lấy chi tiết symbol.`); if (!await setLeverage(symbol, maxLeverage)) throw new Error(`Lỗi đặt đòn bẩy.`); await sleep(200);
        ...
        else if (maxLeverage >= MIN_LEVERAGE_TO_TRADE) { TAKE_PROFIT_MULTIPLIER = 5; STOP_LOSS_MULTIPLIER = 3; for (let i = 1; i <= 8; i++) partialCloseLossSteps.push(i * 50); }
...
async function closeSidewaysGridPosition(gridPosition, reason) {
    ...
    try {
        ...
        if (!posOnEx || Math.abs(parseFloat(posOnEx.positionAmt)) < quantity * 0.95) {
            addLog(`  -> Vị thế ${side} ${symbol} không tồn tại hoặc nhỏ hơn dự kiến trên sàn. Hủy đóng.`);
            sidewaysGrid.activeGridPositions = sidewaysGrid.activeGridPositions.filter(p => p.id !== id);
            success = true;
        } else {
            const sideOrder = (side === 'LONG') ? 'SELL' : 'BUY';
            const actualQtyToClose = Math.min(quantity, Math.abs(parseFloat(posOnEx.positionAmt)));
            await callSignedAPI('/fapi/v1/order', 'POST', {
                symbol: symbol,
                side: sideOrder,
                positionSide: side,
                type: 'MARKET',
                quantity: actualQtyToClose,
                reduceOnly: 'true',
        ...
        try {
            ...
            if (winningPos && losingPos && winningPos.partialCloseLossLevels && winningPos.quantity > 0 && winningPos.initialMargin > 0) {
                ...
                if (currentMocIndex >= winningPos.partialCloseLossLevels.length) {
                    if (losingPos.quantity > 0 && !losingPos.hasClosedAllLossPositionAtLastLevel) { addLog(`[KILL] Lệnh thắng ${winningPos.side} đã qua hết mốc. Đóng lệnh lỗ ${losingPos.side}.`); await closePosition(losingPos.symbol, `Thắng qua hết mốc, đóng lỗ`, losingPos.side); losingPos.hasClosedAllLossPositionAtLastLevel = true; }
                    return;
                }
...
async function checkAndHandleRemainingPosition(symbol) {
    ...
    try {
        ...
        if (remaining.length > 0) {
            ...
            for (const pos of remaining) { await closePosition(pos.symbol, `Dọn dẹp vị thế sót`, pos.positionSide); await sleep(1000); }
    ...
    if(currentBotMode === 'kill'){
        for(const p of [currentLongPosition, currentShortPosition]){
            if(p){
                ...
                killPositionsData.push({ type: 'kill', side: p.side, entry: p.entryPrice?.toFixed(pp), qty: p.quantity?.toFixed(qp), pnl: pnl.toFixed(2), curPrice: p.currentPrice?.toFixed(pp), initQty: p.initialQuantity?.toFixed(qp), closedLossQty: p.closedLossAmount?.toFixed(qp), pairEntry: p.pairEntryPrice?.toFixed(pp), mocIdx: (p.nextPartialCloseLossIndex || 0) + 1, tpPrice: p.takeProfitPrice?.toFixed(pp), slPrice: p.stopLossPrice?.toFixed(pp) });
            }
        }
    }
