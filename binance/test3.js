    ...
    try {
        ...
        if (response && response.status && Array.isArray(response.data)) {
            if (response.status === "running_data_available") {
                ...
                if (!silent) addLog(`VPS1 error (status: ${response.status}): ${response.message || 'Lỗi VPS1'}. Dùng cache cũ (${vps1DataCache.length} coins).`);
                return vps1DataCache.length > 0 ? [...vps1DataCache] : [];
            } else {
                if (!silent) addLog(`VPS1 preparing (status: ${response.status}): ${response.message || 'Chưa có message'}. Dùng cache cũ (${vps1DataCache.length} coins).`);
                return vps1DataCache.length > 0 ? [...vps1DataCache] : [];
            }
        } else {
            if (!silent) addLog(`Lỗi định dạng VPS1. Status: ${response?.status}. Dùng cache cũ (${vps1DataCache.length} coins). Raw: ${rawData.substring(0, 200)}`);
            return vps1DataCache.length > 0 ? [...vps1DataCache] : [];
        }
    } catch (error) {
    ...
    try {
        ...
        for (const coin of vps1Coins) {
            if (!existingPositionsSet.has(coin.symbol) && !blacklistedCoinsThisSession.has(coin.symbol)) {
                addLog(`ĐÃ CHỌN: ${coin.symbol} (Vol: ${coin.changePercent.toFixed(2)}%) vì là coin biến động cao nhất và chưa có vị thế/blacklist.`);
                return coin.symbol;
            }
        }

        addLog("Tất cả coin biến động cao từ VPS1 đều đã có vị thế hoặc trong blacklist. Không chọn coin mới.");
        return null;
    ...
    try {
        ...
        for (const order of openOrders) {
            try {
                ...
                addLog(`  Đã hủy lệnh ${order.orderId} (ClientID: ${order.clientOrderId}).`);
                await sleep(100);
            } catch (innerErr) {
...
async function closeSidewaysGridPosition(gridPosition, reason) {
    ...
    try {
        ...
        if (posOnEx) {
            ...
            if (qtyToClose === 0) {
                addLog(`  -> Lượng đóng mốc ${id} là 0. Bỏ qua.`);
                success = true;
            } else if (qtyToClose > currentPositionSize * 1.05) {
                addLog(`  -> Lượng cần đóng ${qtyToClose} lớn hơn vị thế hiện tại ${currentPositionSize}. Có thể mốc đã được đóng. Dọn dẹp.`);
                success = true;
            } else {
            ...
            addLog(`  -> Không tìm thấy vị thế ${side} ${symbol} trên sàn. Có thể đã được đóng.`);
            success = true;
        }
...
async function handleCoinSwitch(reason) {
    ...
    if (currentCoin) {
        ...
        addLog(`[BLACKLIST] Đã thêm ${currentCoin} vào danh sách loại trừ của phiên này.`);
    }

    if (currentBotMode === 'sideways') {
        await closeAllSidewaysPositionsAndOrders(`Chuyển coin do: ${reason}`);
    } else if (currentBotMode === 'kill') {
        if (currentLongPosition) await closePosition(TARGET_COIN_SYMBOL, `Chuyển coin do: ${reason}`, 'LONG');
        if (currentShortPosition) await closePosition(TARGET_COIN_SYMBOL, `Chuyển coin do: ${reason}`, 'SHORT');
    }
...
async function manageOpenPosition() {
    ...
    if (currentBotMode === 'kill') {
        ...
        try {
            ...
            if (currentMarketPrice >= currentLongPosition.takeProfitPrice) {
                addLog(`[KILL TP] Giá thị trường ${currentMarketPrice} chạm mốc TP ảo của LONG tại ${currentLongPosition.takeProfitPrice}. Đóng vị thế...`);
                await closePosition(TARGET_COIN_SYMBOL, 'TP ảo LONG', 'LONG');
                return;
            }
            if (currentMarketPrice <= currentLongPosition.stopLossPrice) {
                addLog(`[KILL SL] Giá thị trường ${currentMarketPrice} chạm mốc SL ảo của LONG tại ${currentLongPosition.stopLossPrice}. Đóng vị thế...`);
                await closePosition(TARGET_COIN_SYMBOL, 'SL ảo LONG', 'LONG');
                return;
            }
            if (currentMarketPrice <= currentShortPosition.takeProfitPrice) {
                addLog(`[KILL TP] Giá thị trường ${currentMarketPrice} chạm mốc TP ảo của SHORT tại ${currentShortPosition.takeProfitPrice}. Đóng vị thế...`);
                await closePosition(TARGET_COIN_SYMBOL, 'TP ảo SHORT', 'SHORT');
                return;
            }
            if (currentMarketPrice >= currentShortPosition.stopLossPrice) {
                addLog(`[KILL SL] Giá thị trường ${currentMarketPrice} chạm mốc SL ảo của SHORT tại ${currentShortPosition.stopLossPrice}. Đóng vị thế...`);
            ...
            if (losingPos.quantity === 0 && winningPos.quantity > 0 && !winningPos.hasAdjustedSLToSpecificLevel['LosingPosClosed']) {
                ...
                if (winningPos.partialCloseLossLevels && winningPos.partialCloseLossLevels.length > moc5Index) {
                    ...
                    addLog(`[KILL] Lệnh lỗ ${losingPos.side} đã đóng hết. Dời SL ảo lệnh lãi ${winningPos.side} về giá tương đương PNL Mốc 5.`);
                    winningPos.stopLossPrice = slPriceForWinning;
                    winningPos.hasAdjustedSLToSpecificLevel['LosingPosClosed'] = true;
                    addLog(`   SL ảo mới của ${winningPos.side} là ${slPriceForWinning.toFixed(winningPos.pricePrecision)}`);
                    return;
                }
            }

            const isPriceNearEntry = pairEntry > 0 && currentMarketPrice && Math.abs(currentMarketPrice - pairEntry) <= (pairEntry * 0.0005);
            const hasHitMilestone = (currentLongPosition.nextPartialCloseLossIndex > 0) || (currentShortPosition.nextPartialCloseLossIndex > 0);

            if (hasHitMilestone && isPriceNearEntry && !sidewaysGrid.isClearingForSwitch) {
                addLog(`[KILL CHECK] Giá về entry sau khi đã chạm mốc. Đánh giá lại...`);
                await fetchAndCacheTopCoinsFromVPS1(true);
            ...
            if (winningPos && losingPos && winningPos.partialCloseLossLevels && winningPos.quantity > 0 && winningPos.initialMargin > 0) {
                ...
                if (currentMocIndex >= winningPos.partialCloseLossLevels.length) {
                    if (losingPos.quantity > 0 && !losingPos.hasClosedAllLossPositionAtLastLevel) {
                        addLog(`[KILL] Lệnh thắng ${winningPos.side} đã qua hết mốc. Đóng lệnh lỗ ${losingPos.side}.`);
                        await closePosition(losingPos.symbol, `Thắng qua hết mốc, đóng lỗ`, losingPos.side);
            ...
            if (err instanceof CriticalApiError && botRunning) await stopBotLogicInternal(`Lỗi manageOpenPosition (Kill) ${TARGET_COIN_SYMBOL}`);
        }
        ...
        if (currentBotMode === 'kill' && (wasLongClosed || wasShortClosed)) {
            ...
            if (remainingPos) {
                if (totalRealizedPnlFromFills >= 0) {
                    ...
                    addLog(`  Lệnh đã đóng bị LỖ. Lệnh ${remainingPos.side} tiếp tục chạy theo kịch bản của nó.`);
                }
            }
        }
...
async function stopBotLogicInternal(reason = "Lệnh dừng thủ công") {
    ...
    try {
        addLog("Đang kiểm tra và đóng tất cả vị thế còn lại...");
        const allPositions = await callSignedAPI('/fapi/v2/positionRisk', 'GET');
        ...
        addLog(`Lỗi nghiêm trọng khi dọn dẹp tất cả vị thế: ${e.msg || e.message}`);
    }
    ...
    if (retryBotTimeout) {
        clearTimeout(retryBotTimeout);
        retryBotTimeout = null;
        addLog("Đã hủy retry khởi động (nếu có).");
    }
    addLog('--- Bot đã dừng ---');
    return 'Bot đã dừng.';
}
async function checkAndHandleRemainingPosition(symbol) {
    ...
    try {
        ...
        addLog(`Lỗi dọn vị thế sót cho ${symbol}: ${error.msg || error.message}`);
...
app.get('/api/status', async (req, res) => {
    ...
    if (currentBotMode === 'kill' && (currentLongPosition || currentShortPosition)) {
        ...
        posText = ` | Lưới: ${sidewaysGrid.activeGridPositions.length} lệnh (hai chiều). Anchor: ${sidewaysGrid.anchorPrice?.toFixed(pp)}. SLs: ${sidewaysGrid.sidewaysStats.slMatchedCount}, TPs: ${sidewaysGrid.sidewaysStats.tpMatchedCount}`;
    } else {
        posText = " | Vị thế: --";
    }
    statusMsg += posText;
