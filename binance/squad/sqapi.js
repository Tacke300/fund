...
const SETTINGS = {
    ...
    M1_LIMIT: 7.0,    // Ngưỡng biến động 1 phút (ví dụ 3.5%)
    M5_LIMIT: 7.0,    // Ngưỡng biến động 5 phút (ví dụ 7%)
    MAX_TOTAL: 100,     
    MIN_GAP: 6000,    
};
...
/**
 * FIX: Tính toán phần trăm biến động chính xác dựa trên mốc thời gian
 */
function calculateChange(pArr, minutes) {
    ...
    // Tìm nến gần mốc thời gian yêu cầu nhất
    let startPoint = pArr[0];
    for (let i = pArr.length - 1; i >= 0; i--) {
        if (pArr[i].t <= thresholdTime) {
            startPoint = pArr[i];
            break;
        }
    }
...
async function updatePriceLogic(s, p, now) {
    ...
    // Tối ưu: Chỉ giữ tối đa 10 phút dữ liệu để tiết kiệm RAM (600 nến nếu WS gửi mỗi giây)
...
async function postToSquare(symbol, changeText, type) {
    ...
    try {
        ...
        addLog(`✅ [POST] ${symbol} | Lý do: ${changeText} | Tổng: ${state.postsToday}`);
    } catch (e) { 
        addLog(`❌ Lỗi API Square: ${e.response?.data?.message || e.message}`); 
    }
}
...
async function checkNightFill() {
    ...
    if (hour === 23 && state.postsToday < SETTINGS.MAX_TOTAL) {
        addLog(`🌙 Đang quét Top Volume bù cho đủ ${SETTINGS.MAX_TOTAL} bài...`);
        try {
            const response = await axios.get('https://fapi.binance.com/fapi/v1/ticker/24hr');
            const topVol = response.data
        ...
        } catch (e) { addLog("❌ Lỗi REST API Volume: " + e.message); }
    }
...
function initWS() {
    ...
    ws.on('message', (data) => {
        try {
            ...
            const now = Date.now();
            raw.forEach(t => { 
                if (t.s.endsWith('USDT')) updatePriceLogic(t.s, parseFloat(t.c), now); 
            });
        } catch(e) {}
    });
    ws.on('error', (e) => addLog(`⚠️ WS Error: ${e.message}`));
    ws.on('close', () => {
...
app.get('/api/status', (req, res) => {
    const tableData = Object.values(state.coinData)
        .filter(v => v.live)
        .sort((a, b) => Math.abs(b.live.c5) - Math.abs(a.live.c5))
        .slice(0, 25)
        .map(v => ({ s: v.symbol, c1: v.live.c1, c5: v.live.c5 }));
    res.json({ ...state, table: tableData });
});

app.get('/api/toggle', (req, res) => { state.isRunning = !state.isRunning; res.json({ s: state.isRunning }); });

app.get('/', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><script src="https://cdn.tailwindcss.com"></script><style>@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;900&display=swap');body{background:#0b0e11;color:#d4d4d8;}</style></head>
    <body class="p-2 h-screen flex flex-col items-center overflow-hidden">
        <div class="w-full max-w-md h-full flex flex-col">
            <div class="bg-[#1e2329] p-4 rounded-2xl border-b-4 border-yellow-500 mb-3">
                <div class="flex justify-between items-center mb-4">
                    <h1 style="font-family:'Orbitron'" class="text-xl font-black text-yellow-500 italic">LUFFY PRO V4</h1>
            ...
            <div class="bg-[#1e2329] rounded-2xl flex-1 overflow-hidden flex flex-col mb-3 border border-white/5">
                <div class="p-3 bg-white/5 text-[10px] font-bold text-yellow-500 flex justify-between"><span>TICKER REALTIME (TOP 25)</span><span id="active-tag" class="text-green-500">WS ACTIVE</span></div>
                <div class="overflow-y-auto flex-1"><table class="w-full text-[11px]"><tbody id="tb"></tbody></table></div>
            </div>
            <div id="lb" class="h-32 bg-black/80 rounded-xl p-3 text-[9px] font-mono overflow-y-auto text-zinc-400"></div>
        </div>
        <script>
                ...
                try {
                    ...
                    tbody.innerHTML = d.table.map(v => \`
                        <tr class="border-b border-white/5">
                            <td class="p-3 font-bold text-yellow-500">\${v.s}</td>
                            <td class="text-right p-3 \${Math.abs(v.c1) >= 3.5 ? 'text-red-400 font-bold' : 'text-zinc-400'}">M1: \${v.c1}%</td>
                            <td class="text-right p-3 \${Math.abs(v.c5) >= 7 ? 'text-green-400 font-bold' : 'text-zinc-400'}">M5: \${v.c5}%</td>
                        </tr>\`).join('');
                } catch(e) {}
            }
...
app.listen(PORT, '0.0.0.0', () => { initWS(); console.log(`Luffy V4 VIP Ready!`); });
