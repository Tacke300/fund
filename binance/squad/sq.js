...
// --- KHO DỮ LIỆU 1 TRIỆU BIẾN THỂ (MỖI MẢNG 100 CÂU) ---
const intros = [
...
const bodies = [
    "Giá hiện tại đang neo đậu tại mức ổn định.", "Cấu trúc nến cho thấy phe bò đang cố gắng kiểm soát.", "Áp lực bán dường như đã cạn kiệt ở vùng này.", "Xu hướng tăng được củng cố bởi khối lượng giao dịch.", "Mô hình hai đáy đang dần hình thành trên đồ thị.", "Giá đang tích lũy trong một biên độ hẹp.", "Biến động CHANGE% tạo ra biên độ dao động lớn.", "Các chỉ báo kỹ thuật đang tiến sát vùng quá mua.", "Kháng cự ngắn hạn đang ngăn cản đà tăng trưởng.", "Lực cầu bắt đáy xuất hiện mạnh mẽ khi giá giảm.",
    ...
    "Dòng vốn từ các quỹ lớn đang có sự dịch chuyển nhẹ.", "Cơ hội lướt sóng ngắn hạn đang hiện rõ trên chart.", "Tỷ lệ Funding Rate đang ở mức cực kỳ hấp dẫn.", "Giá đang bám sát đường kênh giá tăng trưởng.", "Sự kiện sắp tới sẽ là chất xúc tác cho mức giá này.", "Lực cầu ẩn (Hidden Demand) đang xuất hiện âm thầm.", "Giá đã hoàn thành nhịp chỉnh sóng Elliott.", "Sự đồng thuận của thị trường đang tăng lên.", "Cần vượt qua vùng cản này để tiến tới target xa hơn.", "Thị trường đang có dấu hiệu hạ nhiệt sau đợt tăng nóng.",
...
function generateFinalContent(coin, price, change) {
    ...
    const text = `🔥 [SIGNAL ANALYSIS]: ${coin}\n\n` +
                 `${intro}\n\n` +
                 `${body}\n\n` +
                 `📍 ENTRY: ${entry}\n` +
                 `🎯 TP1: ${tp1}\n` +
...
async function postTaskWithForce() {
    ...
    if (coinQueue.length === 0) {
        try {
            ...
            coinQueue = res.data
                ...
                .map(c => ({ symbol: c.symbol.replace('USDT', ''), price: c.lastPrice, change: c.priceChangePercent }))
                .sort((a, b) => parseFloat(b.price) - parseFloat(a.price));
            logStep(`✅ Đã nạp ${coinQueue.length} coin Futures.`);
    ...
    try {
        ...
        logStep(`📝 Soạn $${currentCoin.symbol}. Nghỉ 5s...`);
        await textbox.click();
        ...
        // 1. Gõ văn bản thuần trước (Không có $ hay # trong bài)
        await page.keyboard.type(content.body, { delay: 5 });

        // 2. Gõ thẻ $ ở cuối (Đợi 1.5s -> Enter -> 2 dấu cách)
        logStep("🏷 Chèn Tags $ cuối bài...");
        for (const symbol of content.tags) {
        ...
        // 3. Gõ thẻ # ở cuối (Dưới dòng $)
        await page.keyboard.press('Enter');
        logStep("🏷 Chèn Tags # cuối bài...");
        const hashes = [currentCoin.symbol, "BinanceSquare", "CryptoAnalysis"];
        ...
        if (await postBtn.isEnabled()) {
            await postBtn.click();
            logStep(`🎯 Đã đăng $${currentCoin.symbol}. Chờ 15s chuyển bài...`);
            totalPosts++;
            history.unshift({ coin: currentCoin.symbol, time: new Date().toLocaleTimeString(), status: 'Thành công' });
        }
    } catch (err) {
        logStep(`❌ Lỗi luồng: ${err.message}`);
    }
}

// --- GIỮ NGUYÊN CÁC HÀM CƠ CHẾ ---
...
app.get('/stop', async (req, res) => { isRunning = false; if (context) await context.close(); mainPage = null; res.json({ status: 'stopped' }); });
app.get('/stats', (req, res) => res.json({ isRunning, totalPosts, history, userInfo }));
app.get('/login', async (req, res) => { if (context) await context.close(); const ctx = await initBrowser(true); const p = await ctx.newPage(); await p.goto('https://www.binance.com/vi/square'); res.send("OK"); });
app.listen(port, '0.0.0.0', () => logStep(`SERVER: ${port}`));
