...
// --- START: BUG FIX ---
// Hàm theo dõi số dư mới, đáng tin cậy hơn.
// Thay thế hoàn toàn hàm pollForBalance cũ.
async function pollForBalanceArrival(exchangeId, amountToReceive, maxPollAttempts = 90, pollIntervalMs = 10000) {
    const exchange = exchanges[exchangeId];
    // Xác định đúng ví nhận tiền (Spot hoặc Main cho KuCoin)
    const targetWalletType = (exchangeId === 'kucoinfutures') ? 'main' : 'spot';
    
    try {
        // 1. Lấy số dư ban đầu của VÍ ĐÍCH
        const initialBalanceData = await exchange.fetchBalance();
        const initialBalance = initialBalanceData[targetWalletType]?.free?.USDT || 0;
        safeLog('log', `[POLL] Bắt đầu theo dõi ví '${targetWalletType}'. Số dư ban đầu: ${initialBalance.toFixed(4)} USDT trên ${exchangeId.toUpperCase()}.`);
        ...
        for (let i = 0; i < maxPollAttempts; i++) {
            ...
            try {
                ...
                // 3. Điều kiện thành công: số dư mới phải lớn hơn số dư cũ một cách hợp lý
                // (trừ hao phí rút tiền, thường không quá 1 USDT)
                if (currentBalance > initialBalance && currentBalance >= initialBalance + (amountToReceive - 2)) {
                    ...
                    // Trả về thông tin chi tiết để các bước sau xử lý chính xác
                    return { found: true, type: targetWalletType, balance: currentBalance };
                }
            } catch (pollError) {
...
async function executeFundTransfer(fromExchangeId, toExchangeId, amount) {
    ...
    try {
        ...
        if (fromExchangeId === 'bitget') {
            params.chain = targetDepositInfo.network;
        }
        ...
        // Sử dụng hàm theo dõi mới, đáng tin cậy
        ...
        try {
            ...
            const availableBalance = fullReceivedBalance[pollResult.type]?.free?.['USDT'] || 0;
            
            if (availableBalance <= 0) {
                 throw new Error(`Số dư khả dụng trong ví '${pollResult.type}' là 0. Không thể chuyển.`);
            }
