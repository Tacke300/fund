...
EXCHANGE_IDS.forEach(id => {
    ...
    exchanges[id].enableRateLimit = true; // Bật giới hạn tỷ lệ yêu cầu của CCXT
});
    ...
    const promises = EXCHANGE_IDS.map(async (id) => {
        ...
        try {
            ...
            if (exchange.has['fetchLeverageTiers']) {
                ...
                await exchange.loadMarkets(true); // Đảm bảo thị trường được tải
            ...
            if (e instanceof ccxt.AuthenticationError) {
                ...
                errorMessage = `LỖI SÀN GIAO DỊCH! ${id.toUpperCase()}: ${e.message}.`;
            }
...
async function masterLoop() {
    ...
    // Cập nhật leverage cache trước
    await initializeLeverageCache(); 
    // Sau đó cập nhật funding rates (sẽ sử dụng leverageCache)
    const freshFundingData = await fetchFundingRatesForAllExchanges(); 
    exchangeData = freshFundingData; // Cập nhật biến toàn cục exchangeData
...
server.listen(PORT, async () => {
    ...
    // Chạy vòng lặp chính lần đầu ngay khi server khởi động
    await masterLoop(); 
    // Thiết lập interval để làm mới leverage cache định kỳ (vì leverage ít thay đổi hơn funding)
