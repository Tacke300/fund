...
const mexcApi = axios.create({ baseURL: 'https://contract.mexc.com', timeout: 15000 });
...
let bot1 = {
    ...
    exchange: new ccxt.mexc({ apiKey: API_KEY, secret: SECRET_KEY, enableRateLimit: true, options: { defaultType: 'swap', adjustForTimeDifference: true } }),
    mexcApi: axios.create({ baseURL: 'https://contract.mexc.com', timeout: 15000 })
};
...
async function callMexcRaw(bot, endpoint, method = 'GET', data = {}) {
    const timestamp = Date.now() + (bot.timestampOffset || 0);
    let query = '';
    let body = null;
    let signStr = '';
    ...
    if (method === 'GET' || method === 'DELETE') {
        if (Object.keys(data).length > 0) {
            ...
            signStr = API_KEY + timestamp + query;
        } else {
            signStr = API_KEY + timestamp;
        }
    } else {
        body = data;
        const jsonBody = typeof data === 'string' ? data : JSON.stringify(data);
        signStr = API_KEY + timestamp + jsonBody;
    }
    ...
    const response = await bot.mexcApi({ method, url, data: body, headers });
    return response.data?.data !== undefined ? response.data.data : response.data;
}

async function mexcPrivate(bot, endpoint, method = 'GET', data = {}) {
    try {
        if (endpoint === '/fapi/v1/openOrders' || endpoint.includes('open_orders')) {
            const sym = data.symbol;
            const url = sym ? `/api/v1/contract/open_orders/${sym}` : '/api/v1/contract/open_orders';
            const res = await callMexcRaw(bot, url, 'GET');
            const list = Array.isArray(res) ? res : (res?.data || []);
            return list.map(o => ({
                orderId: o.orderId || o.id,
                symbol: o.symbol,
                positionSide: o.positionType === 1 ? 'LONG' : (o.positionType === 2 ? 'SHORT' : (o.side === 1 || o.side === 4 ? 'LONG' : 'SHORT')),
                side: o.side
            }));
        }
        if (endpoint === '/fapi/v1/order' && method === 'DELETE') {
            const orderId = data.orderId;
            if (orderId) {
                return await callMexcRaw(bot, '/api/v1/contract/cancel', 'POST', [orderId]);
            }
            return [];
        }
        if (endpoint === '/fapi/v2/positionRisk') {
            const sym = data.symbol;
            const res = await callMexcRaw(bot, '/api/v1/contract/position/open_positions', 'GET', sym ? { symbol: sym } : {});
            const list = Array.isArray(res) ? res : (res?.data || []);
            return list.map(p => ({
                symbol: p.symbol,
                positionSide: p.positionType === 1 ? 'LONG' : 'SHORT',
                positionAmt: p.positionType === 1 ? String(p.holdVol || 0) : String(-(p.holdVol || 0)),
                markPrice: String(p.fairPrice || p.holdAvgPrice || p.implyPrice || 0),
                unRealizedProfit: String(p.unrealised || 0)
            }));
        }
        if (endpoint === '/fapi/v1/userTrades') {
            const sym = data.symbol;
            const res = await callMexcRaw(bot, '/api/v1/contract/deal/history', 'GET', { symbol: sym, page_num: 1, page_size: data.limit || 12 });
            const list = Array.isArray(res) ? res : (res?.data || []);
            return list.map(t => ({
                positionSide: t.positionType === 1 ? 'LONG' : 'SHORT',
                realizedPnl: String(t.realised || t.profit || 0),
                commission: String(t.fee || 0),
                time: t.timestamp || t.createTime || Date.now()
            }));
        }
        if (endpoint === '/fapi/v2/account') {
            const res = await callMexcRaw(bot, '/api/v1/contract/account/assets', 'GET');
            const assets = Array.isArray(res) ? res : (res?.data || []);
            const usdt = assets.find(a => a.currency === 'USDT') || assets[0] || {};
            return {
                totalMarginBalance: String(usdt.equity || usdt.totalBalance || 0),
                availableBalance: String(usdt.availableBalance || usdt.availableMargin || 0),
                totalUnrealizedProfit: String(usdt.unrealised || 0)
            };
        }

        return await callMexcRaw(bot, endpoint, method, data);
    } catch (e) {
        if (e.response?.status === 401 || e.message?.includes('1021') || e.message?.includes('timestamp')) {
            const t = await mexcApi.get('/api/v1/contract/time').catch(() => null);
            if (t?.data?.data) {
                bot.timestampOffset = t.data.data - Date.now();
                return mexcPrivate(bot, endpoint, method, data);
            }
        }
        throw e;
    }
...
async function syncTPSL(bot, symbol, side, info, tpPrice, slPrice) {
    ...
    try {
        const orders = await mexcPrivate(bot, '/fapi/v1/openOrders', 'GET', { symbol });
        for (const o of orders.filter(o => o.positionSide === side)) {
            await mexcPrivate(bot, '/fapi/v1/order', 'DELETE', { symbol, orderId: o.orderId }).catch(()=>{});
        }
        if (tpPrice) await bot.exchange.createOrder(symbol, 'TAKE_PROFIT_MARKET', sideClose, undefined, undefined, { positionSide: side, stopPrice: tpPrice.toFixed(info.pricePrecision), closePosition: true });
        if (slPrice) await bot.exchange.createOrder(symbol, 'STOP_MARKET', sideClose, undefined, undefined, { positionSide: side, stopPrice: slPrice.toFixed(info.pricePrecision), closePosition: true });
    } catch (e) {}
}
...
async function closePositionAndLog(bot, b, markP, reasonStr) {
    ...
    // Khối 1: Gửi lệnh đóng vị thế lên sàn MEXC
    try {
        const posRisk = await mexcPrivate(bot, '/fapi/v2/positionRisk', 'GET', { symbol: b.symbol }).catch(() => []);
    ...
    try {
        ...
        const trades = await mexcPrivate(bot, '/fapi/v1/userTrades', 'GET', { symbol: b.symbol, limit: 12 }).catch(() => []);
    ...
    try {
        const openOrders = await mexcPrivate(bot, '/fapi/v1/openOrders', 'GET', { symbol: b.symbol }).catch(() => []);
        for (const o of openOrders.filter(o => o.positionSide === b.side)) {
            await mexcPrivate(bot, '/fapi/v1/order', 'DELETE', { symbol: b.symbol, orderId: o.orderId }).catch(()=>{});
        }
    } catch (e) {}
}
...
async function panicCloseAll(bot, reasonLog) {
    try {
        const posRisk = await mexcPrivate(bot, '/fapi/v2/positionRisk');
...
async function openPosition(bot, symbol, dcaData = null, forcedSide = null, sharedQty = null, sharedMargin = null, sharedPrice = null, isDiangucSignal = false, signalVols = null) {
    ...
    try {
        ...
        if (isDCA) {
            const ticker = await mexcApi.get(`/api/v1/contract/ticker?symbol=${symbol}`);
            const tickerObj = ticker.data?.data || ticker.data || {};
            currentPrice = parseFloat(tickerObj.fairPrice || tickerObj.lastPrice || 0);
...
async function checkMarginLimits(bot) {
    ...
    const acc = await mexcPrivate(bot, '/fapi/v2/account').catch(() => null);
...
async function buildStatusResponse(bot, cacheObj) {
    ...
    const posRisk = await mexcPrivate(bot, '/fapi/v2/positionRisk').catch(() => []);
...
const handleQuickCloseSymbol = async (bot, req, res) => {
    ...
    if (!foundSide) {
        try {
            const p = (await mexcPrivate(bot, '/fapi/v2/positionRisk', 'GET', { symbol })).find(x => Math.abs(parseFloat(x.positionAmt)) > 0);
    ...
    if (b) {
        ...
        try {
            const p = (await mexcPrivate(bot, '/fapi/v2/positionRisk', 'GET', { symbol })).find(x => x.positionSide === foundSide && Math.abs(parseFloat(x.positionAmt)) > 0);
...
async function init() {
    try {
        ...
        const info = await mexcApi.get('/api/v1/contract/detail');
        const list = info.data?.data || info.data || [];
        const temp = {};
        list.forEach(s => {
            if (s.state !== 0 && s.state !== 1) return; 
            const maxLev = s.maxLeverage || 20;
            if (maxLev < 20) { sharedState.permanentBlacklist[s.symbol] = true; return; }
            const stepSize = Math.pow(10, -(s.volScale || 0));
            temp[s.symbol] = { quantityPrecision: s.volScale || 0, pricePrecision: s.priceScale || 2, stepSize: stepSize, minNotional: 5.0, maxLeverage: maxLev };
        });
...
setInterval(async () => {
    ...
    const posRisk = await mexcPrivate(targetBotForRisk, '/fapi/v2/positionRisk').catch(() => []);
    const exchangeSymbolsWithPositions = new Set(posRisk.filter(p => Math.abs(parseFloat(p.positionAmt)) > 0).map(p => p.symbol));

    let entrySignal = null;
    for (const rawC of sharedState.candidatesList) {
        const cSymbol = normalizeSymbol(rawC.symbol);
        const c = { ...rawC, symbol: cSymbol };
    ...
    if (entrySignal) {
        ...
        if (entrySignal.override) {
            ...
            const forceCloseSymbol = async (bot) => {
                ...
                const pr = await mexcPrivate(bot, '/fapi/v2/positionRisk', 'GET', { symbol }).catch(() => []);
                for (const p of pr) {
        ...
        const acc = await mexcPrivate(targetBotForAcc, '/fapi/v2/account').catch(() => null);
        if (!acc) {
            sharedState.pendingOrders.delete(symbol);
            return; 
        }
        ...
        const ticker = await mexcApi.get(`/api/v1/contract/ticker?symbol=${symbol}`).catch(() => null);
        if (!ticker) {
            sharedState.pendingOrders.delete(symbol);
            return;
        }
        const tickerObj = ticker.data?.data || ticker.data || {};
        const currentPrice = parseFloat(tickerObj.fairPrice || tickerObj.lastPrice || 0);
...
appServer.listen(2222, () => console.log('🌐 [MAIN MASTER] Port 2222'));
appBot1.listen(1731, () => console.log('📈 [BOT 1 UI] Port 1731'));
appBot2.listen(1732, () => console.log('📉 [BOT 2 UI] Port 1732'));
